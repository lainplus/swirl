...
#ifdef SWIRL_USING_DOUBLE_FOR_LDOUBLE
...
#endif
/********************************************************/
#ifndef CONFIG_SWIRL_ASM
ST_FUNC void asm_instr(void)
{
    swirl_error("inline asm() not supported");
}
...
static void gen_inline_functions(SwirlState *s);
static void free_inline_functions(SwirlState *s);
...
/* compiling intel long double natively */
    ...
    && (defined SWIRL_TARGET_I386 || defined SWIRL_TARGET_X86_64)
# define SWIRL_IS_NATIVE_387
#endif
...
{
    if (vtop != vstack - 1)
        swirl_error("internal compiler error: vstack leak (%d)",
                  (int)(vtop - vstack + 1));
}
...
static void swirl_debug_stabs (SwirlState *s1, const char *str, int type, unsigned long value,
                             Section *sec, int sym_index)
{
    ...
    if (debug_info) {
        debug_info->sym =
            (struct debug_sym *)swirl_realloc (debug_info->sym,
                                             sizeof(struct debug_sym) *
                                             (debug_info->n_sym + 1));
        s = debug_info->sym + debug_info->n_sym++;
        s->type = type;
        s->value = value;
        s->str = swirl_strdup(str);
        s->sec = sec;
...
static void swirl_debug_stabn(int type, int value)
{
    if (type == N_LBRAC) {
        struct debug_info *info =
            (struct debug_info *) swirl_mallocz(sizeof (*info));
...
static void swirl_get_debug_info(SwirlState *s1, Sym *s, CString *result)
{
    int type;
    int n = 0;
    int debug_type = -1;
    ...
    if ((type & VT_BTYPE) == VT_STRUCT) {
        ...
        if (debug_type == -1) {
            ...
            cstr_printf (&str, ";");
            swirl_debug_stabs(s1, str.data, N_LSYM, 0, NULL, 0);
            cstr_free (&str);
        }
    }
    ...
    for (;;) {
        ...
        if (type == VT_PTR)
            ...
            swirl_get_debug_info (s1, t->type.ref, result);
            return;
        }
        else
            break;
...
static void swirl_debug_finish (SwirlState *s1, struct debug_info *cur)
{
    while (cur) {
        int i;
        ...
        swirl_debug_finish (s1, cur->child);
...
static void swirl_add_debug_info(SwirlState *s1, int param, Sym *s, Sym *e)
{
    CString debug_str;
    cstr_new (&debug_str);
    for (; s != e; s = s->prev) {
        ...
        swirl_get_debug_info(s1, s, &debug_str);
        swirl_debug_stabs(s1, debug_str.data, param ? N_PSYM : N_LSYM, s->c, NULL, 0);
    }
    cstr_free (&debug_str);
}

static void swirl_debug_extern_sym(SwirlState *s1, Sym *sym, int sh_num, int sym_bind)
{
    ...
    swirl_get_debug_info(s1, sym, &str);
    if (sym_bind == STB_GLOBAL)
        swirl_debug_stabs(s1, str.data, N_GSYM, 0, NULL, 0);
    else
        swirl_debug_stabs(s1, str.data,
            (sym->type.t & VT_STATIC) && data_section == s
            ? N_STSYM : N_LCSYM, 0, s, sym->c);
    cstr_free (&str);
}

static void swirl_debug_typedef(SwirlState *s1, Sym *sym)
{
    CString str;
...
ST_FUNC void swirl_debug_end(SwirlState *s1)
{
    ...
    put_stabs_r(s1, NULL, N_SO, 0, 0,
        text_section->data_offset, text_section, section_sym);
    swirl_free(debug_hash);
}

static BufferedFile* put_new_file(SwirlState *s1)
{
    BufferedFile *f = file;
...
ST_FUNC void swirl_debug_funcstart(SwirlState *s1, Sym *sym)
{
    ...
    debug_info = NULL;
    swirl_debug_stabn(N_LBRAC, ind - func_ind);
    if (!(f = put_new_file(s1)))
        return;
    cstr_new (&debug_str);
    cstr_printf(&debug_str, "%s:%c", funcname, sym->type.t & VT_STATIC ? 'f' : 'F');
    swirl_get_debug_info(s1, sym->type.ref, &debug_str);
...
ST_FUNC void swirl_debug_funcend(SwirlState *s1, int size)
{
    if (!s1->do_debug)
        return;
    swirl_debug_stabn(N_RBRAC, size);
    swirl_debug_finish (s1, debug_info_root);
}
...
/* ------------------------------------------------------------------------- */
/* initialize vstack and types.  This must be done also for swirl -E */
                            ...
                            int can_add_underscore)
{
    ...
    if (!sym->c) {
        ...
        if (swirl_state->leading_underscore && can_add_underscore) {
            buf1[0] = '_';
        ...
        if (swirl_state->do_debug
            && sym_type != STT_FUNC
            && sym->v < SYM_FIRST_ANOM)
            swirl_debug_extern_sym(swirl_state, sym, sh_num, sym_bind);
...
static Sym *__sym_malloc(void)
{
    ...
    sym_pool = swirl_malloc(SYM_POOL_NB * sizeof(Sym));
    ...
    if (!(v & SYM_FIELD) && (v & ~SYM_STRUCT) < SYM_FIRST_ANOM) {
        ...
        *ps = s;
        ...
        if (s->prev_tok && sym_scope(s->prev_tok) == s->sym_scope)
            swirl_error("redeclaration of '%s'",
...
static void vsetc(CType *type, int r, CValue *vc)
{
    if (vtop >= vstack + (VSTACK_SIZE - 1))
        swirl_error("memory full (vstack)");
    vcheck_cmp();
    vtop++;
    vtop->type = *type;
    vtop->r = r;
...
static void patch_type(Sym *sym, CType *type)
{
    if (!(type->t & VT_EXTERN) || IS_ENUM_VAL(sym->type.t)) {
        if (!(sym->type.t & VT_EXTERN))
            swirl_error("redefinition of '%s'", get_tok_str(sym->v, NULL));
    ...
    if (!is_compatible_types(&sym->type, type)) {
        swirl_error("incompatible types for redefinition of '%s'",
                  get_tok_str(sym->v, NULL));
        ...
        if ((type->t & VT_STATIC) && !static_proto
            ...
            swirl_warning("static storage ignored for redefinition of '%s'",
                get_tok_str(sym->v, NULL));
        ...
        if ((type->t ^ sym->type.t) & VT_STATIC)
            swirl_warning("storage mismatch for redefinition of '%s'",
                get_tok_str(sym->v, NULL));
    }
}
...
static void patch_storage(Sym *sym, AttributeDef *ad, CType *type)
{
    ...
    if (sym->a.dllimport != ad->a.dllimport)
        swirl_error("incompatible dll linkage for redefinition of '%s'",
            get_tok_str(sym->v, NULL));
#endif
...
{
#ifdef CONFIG_SWIRL_BCHECK
    if (swirl_state->do_bounds_check && !ellipsis && !keep)
        add_local_bounds(*ptop, b);
#endif
    if (swirl_state->do_debug)
        swirl_add_debug_info (swirl_state, !local_scope, *ptop, b);
    sym_pop(ptop, b, keep);
}
        ...
        case TOK_UMOD:
            ...
            if (l2 == 0) {
                if (const_wanted && !(nocode_wanted & unevalmask))
                    swirl_error("division by zero in constant");
                goto general_case;
            }
            switch(op) {
...
#if defined SWIRL_TARGET_X86_64 || defined SWIRL_TARGET_I386
...
void gen_negf(int op)
{
    /* In IEEE negate(x) isn't subtract(0,x).  Without NaNs it's
       ...
       to do some type reinterpretation for this, which SWIRL can do
       only via memory.  */
    ...
    if (bt1 == VT_VOID || bt2 == VT_VOID) {
        ...
        /* If one is a null ptr constant the result type is the other.  */
            ...
            if ((op == '?' || TOK_ISCOND(op))
              ...
              swirl_warning("pointer/integer mismatch in %s",
...
redo:
	...
	goto redo;
        ...
        swirl_error_noabort("invalid operand types for binary operation");
        vpop();
    } else if (bt1 == VT_PTR || bt2 == VT_PTR) {
        ...
        if (bt1 == VT_PTR && bt2 == VT_PTR) {
            if (op != '-')
                swirl_error("cannot use pointers here");
            ...
            if (vtop[-1].type.t & VT_VLA)
                ...
                if (u < 0)
                    swirl_error("unknown array element size");
#if PTR_SIZE == 8
                vpushll(u);
#else
            ...
            gen_op('*');
#ifdef CONFIG_SWIRL_BCHECK
            if (swirl_state->do_bounds_check && !const_wanted) {
        ...
        if (is_float(combtype.t)
            && op != '+' && op != '-' && op != '*' && op != '/'
            && !TOK_ISCOND(op))
            swirl_error("invalid operands for binary operation");
...
#if defined SWIRL_TARGET_ARM64 || defined SWIRL_TARGET_RISCV64 || defined SWIRL_TARGET_ARM
...
#if defined SWIRL_TARGET_ARM64 || defined SWIRL_TARGET_RISCV64
...
again:
    if (sbt != dbt) {
        ...
        c = (vtop->r & (VT_VALMASK | VT_LVAL | VT_SYM)) == VT_CONST;
#if !defined SWIRL_IS_NATIVE && !defined SWIRL_IS_NATIVE_387
        ...
        if (c) {
            ...
            && (vtop->r & (VT_VALMASK | VT_LVAL | VT_SYM))
                == (VT_CONST | VT_SYM)) {
            /* addresses are considered non-zero (see swirltest.c:sinit23) */
            vtop->r = VT_CONST;
            vtop->c.i = 1;
            goto done;
        }
        ...
        if (IS_ENUM(type->t) && type->ref->c < 0)
            swirl_error("cast to incomplete type");

        /* same size and no sign conversion needed */
        if (ds == ss && ds >= 4)
            goto done;
        if (dbt_bt == VT_PTR || sbt_bt == VT_PTR) {
            swirl_warning("cast between pointer and integer of different size");
            if (sbt_bt == VT_PTR) {
        ...
        if (ds == 8) {
            ...
            if (sbt & VT_UNSIGNED) {
#if defined(SWIRL_TARGET_RISCV64)
        ...
        if (ds >= ss)
            goto done;
#if defined SWIRL_TARGET_I386 || defined SWIRL_TARGET_X86_64 || defined SWIRL_TARGET_ARM64
        if (ss == 4) {
            gen_cvt_csti(dbt);
            goto done;
        }
...
static void verify_assign_cast(CType *dt)
{
    ...
    sbt = st->t & VT_BTYPE;
    if (dt->t & VT_CONSTANT)
        swirl_warning("assignment of read-only location");
    switch(dbt) {
    case VT_VOID:
        if (sbt != dbt)
            swirl_error("assignment to void expression");
        break;
    case VT_PTR:
        ...
        if (is_integer_btype(sbt)) {
            swirl_warning("assignment makes pointer from integer without a cast");
            break;
        }
		...
		/* Like GCC don't warn by default for merely changes
		   ...
		   and signed int targets.  */
                ...
                swirl_warning("assignment from incompatible pointer type");
                break;
            }
        }
        if (qualwarn)
            swirl_warning("assignment discards qualifiers from pointer target type");
        break;
    case VT_BYTE:
    case VT_SHORT:
    case VT_INT:
    case VT_LLONG:
        if (sbt == VT_PTR || sbt == VT_FUNC) {
            swirl_warning("assignment makes integer from pointer without a cast");
    ...
    if (sbt == VT_STRUCT) {
        ...
        --vtop;
            ...
            if (delayed_cast) {
                ...
                //swirl_warning("deley cast %x -> %x", sbt, dbt);
...
redo:
	...
	case TOK_CLEANUP2:
	{
	    ...
	    s = sym_find(tok);
	    if (!s) {
	      swirl_warning("implicit declaration of function '%s'",
			  get_tok_str(tok, &tokc));
	      s = external_global_sym(tok, &func_old_type);
            } else if ((s->type.t & VT_BTYPE) != VT_FUNC)
                swirl_error("'%s' is not declared as function", get_tok_str(tok, &tokc));
	    ad->cleanup_func = s;
	    next();
            skip(')');
	    break;
	    ...
	    parse_mult_str(&astr, "section name");
            ad->section = find_section(swirl_state, (char *)astr.data);
            skip(')');
	    cstr_free(&astr);
            break;
        case TOK_ALIAS1:
	...
	case TOK_VISIBILITY2:
        ...
        case TOK_ALIGNED2:
            if (tok == '(') {
                ...
                n = expr_const();
                if (n <= 0 || (n & (n - 1)) != 0) 
                    swirl_error("alignment must be a positive power of two");
                skip(')');
            } else {
                n = MAX_ALIGN;
            }
	    ...
	    if (n != 1 << (ad->a.aligned - 1))
	      swirl_error("alignment of %d is larger than implemented", n);
            break;
        case TOK_PACKED1:
        case TOK_PACKED2:
            ad->a.packed = 1;
        ...
        case TOK_UNUSED1:
        case TOK_UNUSED2:
            /* currently, no need to handle it because swirl does not
        ...
        case TOK_MODE:
            ...
            switch(tok) {
                ...
                case TOK_MODE_word:
                    ...
                    break;
                default:
                    swirl_warning("__mode__(%s) not supported\n", get_tok_str(tok, NULL));
                    break;
            }
            next();
            skip(')');
        ...
        case TOK_DLLIMPORT:
            ...
            break;
        default:
            if (swirl_state->warn_unsupported)
                swirl_warning("'%s' attribute ignored", get_tok_str(t, NULL));
            /* skip parameters */
            if (tok == '(') {
                int parenthesis = 0;
                do {
    ...
    while ((s = s->next) != NULL) {
        ...
        if (v < SYM_FIRST_ANOM) {
            ...
            if (check && (ts->tok & SYM_FIELD))
                swirl_error("duplicate member '%s'", get_tok_str(v, NULL));
            ts->tok ^= SYM_FIELD;
...
static void struct_layout(CType *type, AttributeDef *ad)
{
    ...
    int pcc = !swirl_state->ms_bitfields;
    int pragma_pack = *swirl_state->pack_stack_ptr;
    Sym *f;
    ...
    if (tok != '{') {
        ...
        if (s && (s->sym_scope == local_scope || tok != '{')) {
            ...
            if (u == VT_ENUM && IS_ENUM(s->type.t))
                goto do_decl;
            swirl_error("redefinition of '%s'", get_tok_str(v, NULL));
        }
    } else {
        v = anon_sym++;
    }
...
do_decl:
    ...
    if (tok == '{') {
        next();
        if (s->c != -1)
            swirl_error("struct/union/enum already defined");
        s->c = -2;
        /* cannot be empty */
	    ...
	    CType t;
            ...
            for(;;) {
                ...
                ss = sym_find(v);
                if (ss && !local_stack)
                    swirl_error("redefinition of enumerator '%s'",
                              get_tok_str(v, NULL));
                next();
                if (tok == '=') {
                    next();
		    ...
		    continue;
                ...
                while (1) {
		    if (flexible)
		        swirl_error("flexible array member '%s' not at the end of struct",
                              get_tok_str(v, NULL));
                    bit_size = -1;
                    v = 0;
                    type1 = btype;
			...
			if (tok != ';')
				...
				int v = btype.ref->v;
				if (!(v & SYM_FIELD) && (v & ~SYM_STRUCT) < SYM_FIRST_ANOM) {
				    if (swirl_state->ms_extensions == 0)
                        		expect("identifier");
				}
                    	    }
                        }
			    ...
			    if ((u == VT_STRUCT) && (type1.t & VT_ARRAY) && c)
			        ...
			        swirl_error("field '%s' has incomplete type",
                                      get_tok_str(v, NULL));
                        }
                        if ((type1.t & VT_BTYPE) == VT_FUNC ||
			    (type1.t & VT_BTYPE) == VT_VOID ||
                            (type1.t & VT_STORAGE))
                            swirl_error("invalid type for '%s'", 
                                  get_tok_str(v, NULL));
                    }
                    if (tok == ':') {
                        next();
                        ...
                        if (bit_size < 0)
                            swirl_error("negative width in bit-field '%s'", 
                                  get_tok_str(v, NULL));
                        if (v && bit_size == 0)
                            swirl_error("zero width for bit-field '%s'", 
                                  get_tok_str(v, NULL));
			parse_attribute(&ad1);
                    }
                    ...
                    if (bit_size >= 0) {
                        ...
                        if (bt != VT_INT && 
                            ...
                            bt != VT_BOOL &&
                            bt != VT_LLONG)
                            swirl_error("bitfields must have scalar type");
                        bsize = size * 8;
                        if (bit_size > bsize) {
                            swirl_error("width of '%s' exceeds its type",
                                  get_tok_str(v, NULL));
                            ...
                            swirl_error("field width 64 not implemented");
                        } else {
	    ...
	    parse_attribute(&ad);
            if (ad.cleanup_func) {
                swirl_warning("attribute '__cleanup__' ignored on type");
            }
	    check_fields(type, 1);
	    check_fields(type, 0);
    ...
    while(1) {
        ...
        basic_type1:
            if (u == VT_SHORT || u == VT_LONG) {
                if (st != -1 || (bt != -1 && bt != VT_INT))
                    tmbt: swirl_error("too many basic types");
                st = u;
            } else {
        ...
        case TOK_SIGNED3:
            if ((t & (VT_DEFSIGN|VT_UNSIGNED)) == (VT_DEFSIGN|VT_UNSIGNED))
                swirl_error("signed and unsigned modifier");
            t |= VT_DEFSIGN;
            next();
            typespec_found = 1;
            break;
       ...
       storage:
            if (t & (VT_EXTERN|VT_STATIC|VT_TYPEDEF) & ~g)
                swirl_error("multiple storage classes");
            t |= g;
            next();
            break;
        case TOK_INLINE1:
...
the_end:
    if (swirl_state->char_is_unsigned) {
            ...
            for(;;) {
                ...
                if (l != FUNC_OLD) {
                    ...
                    if ((pt.t & VT_BTYPE) == VT_VOID)
                        swirl_error("parameter declared as void");
                } else {
                    n = tok;
                    if (n < TOK_UIDENT)
                        expect("identifier");
	...
	while (1) {
		...
		gexpr();
            ...
            if ((vtop->r & (VT_VALMASK | VT_LVAL | VT_SYM)) == VT_CONST) {
                n = vtop->c.i;
                if (n < 0)
                    swirl_error("invalid array size");
            } else {
                if (!is_integer_btype(vtop->type.t & VT_BTYPE))
                    swirl_error("size of variable length array should be an integer");
                n = 0;
                t1 = VT_VLA;
            }
        }
        ...
        if ((type->t & VT_BTYPE) == VT_FUNC)
            swirl_error("declaration of an array of functions");
        if ((type->t & VT_BTYPE) == VT_VOID
            || type_size(type, &unused_align) < 0)
            swirl_error("declaration of an array of incomplete type elements");

        t1 |= type->t & VT_VLA;

        if (t1 & VT_VLA) {
            if (n < 0)
              swirl_error("need explicit inner array size in VLAs");
...
#ifdef CONFIG_SWIRL_BCHECK
        if (swirl_state->do_bounds_check)
            vtop->r |= VT_MUSTBOUND;
#endif
    }
}
...
static void gfunc_param_typed(Sym *func, Sym *arg)
{
    ...
    if (func_type == FUNC_OLD ||
        ...
        swirl_error("too many arguments to function");
    } else {
        type = arg->type;
...
ST_FUNC void unary(void)
{
 ...
 push_tokc:
    ...
    str_init:
        if (swirl_state->warn_write_strings)
            t |= VT_CONSTANT;
        type.t = t;
        mk_pointer(&type);
        type.t |= VT_ARRAY;
    ...
    case '(':
	    ...
	    int saved_nocode_wanted = nocode_wanted;
            if (const_wanted && !(nocode_wanted & unevalmask))
                expect("constant");
            if (0 == local_scope)
                swirl_error("statement expression outside of function");
            /* save all registers */
            save_regs(0);
    ...
    case '+':
        ...
        unary();
        if ((vtop->type.t & VT_BTYPE) == VT_PTR)
            swirl_error("pointer not accepted for unary plus");
    ...
    case TOK_ALIGNOF3:
        ...
        if (t == TOK_SIZEOF) {
            if (!(type.t & VT_VLA)) {
                if (size < 0)
                    swirl_error("sizeof applied to an incomplete type");
                vpushs(size);
            } else {
                vla_runtime_type_size(&type, &align);
            }
    ...
    case TOK_builtin_return_address:
        {
            ...
            skip('(');
            if (tok != TOK_CINT) {
                swirl_error("%s only takes positive integers",
    ...
    case TOK_builtin_va_start:
        ...
        if (r == VT_LLOCAL)
            r = VT_LOCAL;
        if (r != VT_LOCAL)
            swirl_error("__builtin_va_start expects a local variable");
        gen_va_start();
	vstore();
        break;
#endif
	...
	if (!str) {
	    ...
	    swirl_error("type '%s' does not match any association", buf);
	}
	begin_macro(str, 1);
	next();
	expr_eq();
...
special_math_val:
    ...
    tok_identifier:
        ...
        if (!s || IS_ASM_SYM(s)) {
            ...
            if (tok != '(')
                swirl_error("'%s' undeclared", name);
            /* for simple function calls, we tolerate undeclared
               external reference to int() function */
            if (swirl_state->warn_implicit_function_declaration
#ifdef SWIRL_TARGET_PE
                ...
                swirl_warning("implicit declaration of function '%s'", name);
    ...
    while (1) {
	    ...
	    s = find_field(&vtop->type, tok, &cumofs);
            if (!s)
                swirl_error("field not found: %s",  get_tok_str(tok & ~SYM_FIELD, &tokc));
            ...
            if (sa)
                swirl_error("too few arguments to function");
            skip(')');
            gfunc_call(nb_args);
            ...
            if (ret_nregs < 0) {
                ...
                /* Promote char/short return values. This is matters only
                   for calling function that were not compiled by SWIRL and
...
static void expr_cond(void)
{
	...
	c = condition_3way();
        ...
        if (c < 0 && is_cond_bool(vtop) && is_cond_bool(&sv)) {
            ...
            gvtst_set(1, t2);
            nocode_wanted = ncw_prev;
            //  swirl_warning("two conditions expr_cond");
            return;
        }
...
ST_FUNC int expr_const(void)
{
    ...
    if (c != wc && (unsigned)c != wc)
        swirl_error("constant exceeds 32 bit");
    return c;
}
...
static void check_func_return(void)
{
    ...
    if (!strcmp (funcname, "main")
        ...
        swirl_warning("function might return no value: '%s'", funcname);
    }
}
...
void prev_scope(struct scope *o, int is_expr)
{
    ...
    if (swirl_state->do_debug)
        swirl_debug_stabn(N_RBRAC, ind - func_ind);
}
...
again:
        ...
        if (tok != ';') {
            ...
            if (b) {
                ...
                if (vtop->type.t != VT_VOID)
                    swirl_warning("void function returns a value");
                vtop--;
            }
        } else if (b) {
            swirl_warning("'return' with no value");
            b = 0;
        }
	...
	gsym_addr(c, d);
        ...
        for (b = 1; b < sw->n; b++)
            if (sw->sv.type.t & VT_UNSIGNED
                ...
                swirl_error("duplicate case value");
        ...
        struct case_t *cr = swirl_malloc(sizeof(struct case_t));
        if (!cur_switch)
            expect("switch");
        ...
        if (gnu_ext && tok == TOK_DOTS) {
            ...
            if ((!(cur_switch->sv.type.t & VT_UNSIGNED) && cr->v2 < cr->v1)
                ...
                swirl_warning("empty case range");
        }
        cr->sym = gind();
	    ...
	    next();
            ...
            if (s) {
                if (s->r == LABEL_DEFINED)
                    swirl_error("duplicate label '%s'", get_tok_str(s->v, NULL));
                s->r = LABEL_DEFINED;
		if (s->next) {
    ...
    block_after_label:
            ...
            if (tok == '}') {
                swirl_warning("deprecated use of label at end of compound statement");
            } else {
                goto again;
            }
...
static void skip_or_save_block(TokenString **str)
{
	...
	if (tok == TOK_EOF) {
	     if (str || level > 0)
	       swirl_error("unexpected end of file");
	     else
	       break;
	}
	if (str)
    ...
    case EXPR_CONST:
	   ...
	   (compound literals).  */
        if (((vtop->r & (VT_VALMASK | VT_LVAL)) != VT_CONST
            ...
            swirl_error("initializer element is not constant");
        break;
    case EXPR_ANY:
        expr_eq();
        break;
...
static void init_assert(init_params *p, int offset)
{
    if (p->sec ? !NODATA_WANTED && offset > p->sec->data_offset
        ...
        swirl_internal_error("initializer overflow");
}
#else
...
static void decl_design_flex(init_params *p, Sym *ref, int index)
{
    if (ref == p->flex_array_ref) {
        ...
        swirl_error("flexible array has zero size in this context");
}
                           ...
                           Sym **cur_field, int flags, int al)
{
    ...
    while (nb_elems == 1 && (tok == '[' || tok == '.')) {
        if (tok == '[') {
	        ...
	        swirl_error("index exceeds array bounds or range is empty");
            if (cur_field)
    ...
    no_designator:
	    ...
	    index = (*cur_field)->c;
            ...
            if (index >= s->c)
                swirl_error("too many initializers");
            type = pointed_type(type);
            elem_size = type_size(type, &align);
            c += index * elem_size;
        } else {
...
static void init_putv(init_params *p, CType *type, unsigned long c)
{
    ...
    if (sec) {
        ...
        if ((vtop->r & VT_SYM)
            ...
            swirl_error("initializer element is not computable at load time");
	...
	if ((vtop->r & (VT_SYM|VT_CONST)) == (VT_SYM|VT_CONST) &&
	    ...
	    ssec = swirl_state->sections[esym->st_shndx];
		...
		*(double *)ptr = vtop->c.d;
		break;
	    case VT_LDOUBLE:
#if defined SWIRL_IS_NATIVE_387
...
#ifndef SWIRL_CROSS_TEST
                else
                    swirl_error("can't cross compile long double constants");
#endif
		break;
#if PTR_SIZE != 8
	    case VT_LLONG:
	...
	is_compatible_unqualified_types(type, &vtop->type)) {
	    ...
	    len = 0;
            ...
            if (size1 != (tok == TOK_STR ? 1 : sizeof(nwchar_t)))
              swirl_error("unhandled string literal merging");
            ...
            if (!(flags & DIF_SIZE_ONLY)) {
                ...
                if (len < nb)
                    nb = len;
                if (len > nb)
                  swirl_warning("initializer-string for array is too long");
                                   ...
                                   int has_init, int v, int scope)
{
    ...
    int saved_nocode_wanted = nocode_wanted;
#ifdef CONFIG_SWIRL_BCHECK
    int bcheck = swirl_state->do_bounds_check && !NODATA_WANTED;
#endif
    init_params p = {0};
...
#if defined SWIRL_TARGET_PE && defined SWIRL_TARGET_X86_64
...
static void gen_function(Sym *sym)
{
    ...
    if (sym->type.ref->f.func_ctor)
        add_array (swirl_state, ".init_array", sym->c);
    if (sym->type.ref->f.func_dtor)
        add_array (swirl_state, ".fini_array", sym->c);
    ...
    /* put debug symbol */
    swirl_debug_funcstart(swirl_state, sym);
    ...
    /* end of function */
    swirl_debug_funcend(swirl_state, ind - func_ind);
    ...
    do {
        ...
        for (i = 0; i < s->nb_inline_fns; ++i) {
            ...
            if (sym && (sym->c || !(sym->type.t & VT_INLINE))) {
                ...
                fn->sym = NULL;
                swirl_debug_putfile(s, fn->filename);
...
static int decl0(int l, int is_for_loop_init, Sym *func_sym)
{
	...
	if (tok == TOK_STATIC_ASSERT) {
	    ...
	    if (tok == ')') {
		if (!c)
		    swirl_error("_Static_assert fail");
		next();
	    ...
	    if (c == 0)
		swirl_error("%s", (char *)error_str.data);
	    cstr_free(&error_str);
	    skip(')');
	  static_assert_out:
            skip(';');
		...
		v = btype.ref->v;
		if (!(v & SYM_FIELD) && (v & ~SYM_STRUCT) >= SYM_FIRST_ANOM)
        	    swirl_warning("unnamed struct/union that defines no instances");
                next();
                continue;
	    }
            if (IS_ENUM(btype.t)) {
	    ...
	    ad = adbase;
            ...
            if ((type.t & VT_BTYPE) == VT_FUNC) {
                if ((type.t & VT_STATIC) && (l == VT_LOCAL))
                    swirl_error("function without file scope cannot be static");
                ...
                swirl_warning("type defaults to int");
            }
            ...
            if (ad.a.dllimport || ad.a.dllexport) {
                if (type.t & VT_STATIC)
                    swirl_error("cannot have dll linkage with static");
                if (type.t & VT_TYPEDEF) {
                    swirl_warning("'%s' attribute ignored for typedef",
            ...
            if (tok == '{') {
                if (l != VT_CONST)
                    swirl_error("cannot use local functions");
                if ((type.t & VT_BTYPE) != VT_FUNC)
                ...
                if (sym->type.t & VT_INLINE) {
                    struct InlineFunc *fn;
                    fn = swirl_malloc(sizeof *fn + strlen(file->filename));
                    strcpy(fn->filename, file->filename);
                    fn->sym = sym;
		    skip_or_save_block(&fn->func_str);
                    dynarray_add(&swirl_state->inline_fns,
				 &swirl_state->nb_inline_fns, fn);
                } else {
		...
		if (l == VT_CMP) {
			...
			if ((sym->v & ~SYM_FIELD) == v)
			    goto found;
		    swirl_error("declaration for parameter '%s' but no such parameter",
			      get_tok_str(v, NULL));
found:
		    if (type.t & VT_STORAGE) /* 'register' is okay */
		        swirl_error("storage class specified for '%s'",
				  get_tok_str(v, NULL));
		    if (sym->type.t != VT_VOID)
		        swirl_error("redefinition of parameter '%s'",
				  get_tok_str(v, NULL));
		    ...
		    sym->type = type;
                    ...
                    if (sym && sym->sym_scope == local_scope) {
                        if (!is_compatible_types(&sym->type, &type)
                            ...
                            swirl_error("incompatible redefinition of '%s'",
                                get_tok_str(v, NULL));
                        sym->type = type;
                    } else {
                    ...
                    sym->f = ad.f;
                    if (swirl_state->do_debug)
                        swirl_debug_typedef (swirl_state, sym);
		} else if ((type.t & VT_BTYPE) == VT_VOID
			   && !(type.t & VT_EXTERN)) {
		    swirl_error("declaration of void object");
                } else {
                    r = 0;
                    if ((type.t & VT_BTYPE) == VT_FUNC) {
                    ...
                    if (has_init && (type.t & VT_VLA))
                        swirl_error("variable length array cannot be initialized");
		        ...
		        || (type.t & VT_BTYPE) == VT_FUNC
                        ...
                        if (ad.alias_target) {
                            ...
                            if (!esym)
                                swirl_error("unsupported forward __alias__ attribute");
