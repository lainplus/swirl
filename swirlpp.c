...
static const char swirl_keywords[] = 
...
{
    if (tok != c)
        swirl_error("'%c' expected (got \"%s\")", c, get_tok_str(tok, &tokc));
    next();
}

ST_FUNC void expect(const char *msg)
{
    swirl_error("%s expected", msg);
}
...
#ifndef USE_TAL
#define tal_free(al, p) swirl_free(p)
#define tal_realloc(al, p, size) swirl_realloc(p, size)
...
{
    TinyAlloc *al = swirl_mallocz(sizeof(TinyAlloc));
    al->p = al->buffer = swirl_malloc(size);
    al->limit = limit;
    al->size = size;
    if (pal) *pal = al;
    return al;
...
static void cstr_realloc(CString *cstr, int new_size)
{
    ...
    while (size < new_size)
        size = size * 2;
    cstr->data = swirl_realloc(cstr->data, size);
...
static TokenSym *tok_alloc_new(TokenSym **pts, const char *str, int len)
{
    ...
    if (tok_ident >= SYM_FIRST_ANOM) 
        swirl_error("memory full (symbols)");

    /* expand token table if needed */
    i = tok_ident - TOK_IDENT;
    if ((i % TOK_ALLOC_INCR) == 0) {
        ptable = swirl_realloc(table_ident, (i + TOK_ALLOC_INCR) * sizeof(TokenSym *));
        table_ident = ptable;
    }
        ...
        if (c == '\n') {
            ...
            for(;;) {
                ...
                if (c == '*') {
                    ...
                    p = file->buf_ptr;
                    if (c == CH_EOF)
                        swirl_error("unexpected end of file in comment");
                    if (c == '\\') {
    ...
    for(;;) {
        ...
        if (c == sep) {
            ...
            unterminated_string:
                ...
                swirl_error("missing terminating %c character", sep);
            } else if (c == '\\') {
                /* escape : just skip \[\r]\n */
                PEEKC_EOB(c, p);
                if (c == '\n') {
...
ST_INLN void define_push(int v, int macro_type, int *str, Sym *first_arg)
{
	...
	swirl_warning("%s redefined", get_tok_str(v, NULL));
}
...
ST_FUNC void label_pop(Sym **ptop, Sym *slast, int keep)
{
    ...
    for(s = *ptop; s != slast; s = s1) {
        ...
        if (s->r == LABEL_DECLARED) {
            swirl_warning("label '%s' declared but not used", get_tok_str(s->v, NULL));
        } else if (s->r == LABEL_FORWARD) {
                swirl_error("label '%s' used but not defined",
...
/* fake the nth "#if defined test_..." for swirl -dt -run */
static void maybe_run_test(SwirlState *s)
{
    const char *p;
    ...
    while (tok != TOK_LINEFEED && tok != TOK_EOF) {
      ...
      redo:
        if (tok == TOK_DEFINED) {
            ...
            if (tok < TOK_IDENT)
                expect("identifier");
            if (swirl_state->run_test)
                maybe_run_test(swirl_state);
            c = define_find(tok) != 0;
            if (t == '(') {
                next_nomacro();
                if (tok != ')')
            ...
            next();
            if (tok == '(')
                swirl_error("function-like macro '%s' is not defined",
                          get_tok_str(t, NULL));
            goto redo;
        }
        tok_str_add_tok(str);
...
ST_FUNC void parse_define(void)
{
    ...
    if (v < TOK_IDENT || v == TOK_DEFINED)
        swirl_error("invalid macro name '%s'", get_tok_str(tok, &tokc));
    ...
    if (tok == '(') {
        ...
        if (tok != ')') for (;;) {
            ...
            if (varg < TOK_IDENT)
        bad_list:
                swirl_error("bad macro parameter list");
    ...
    if (3 == spc)
bad_twosharp:
        swirl_error("'##' cannot appear at either end of macro");
    define_push(v, t, tok_str_dup(&tokstr_buf), first);
}

static CachedInclude *search_cached_include(SwirlState *s1, const char *filename, int add)
{
    ...
    e = swirl_malloc(sizeof(CachedInclude) + strlen(filename));
...
static void pragma_parse(SwirlState *s1)
{
    next_nomacro();
    if (tok == TOK_push_macro || tok == TOK_pop_macro) {
        ...
        if (s)
            ...
            swirl_warning("unbalanced #pragma pop_macro");
        pp_debug_tok = t, pp_debug_symv = v;

    } else if (tok == TOK_once) {
        search_cached_include(s1, file->filename, 1)->once = pp_once;

    } else if (s1->output_type == SWIRL_OUTPUT_PREPROCESS) {
        /* swirl -E: keep pragmas below unchanged */
        unget_tok(' ');
        ...
        if (tok != TOK_STR)
            goto pragma_err;
        p = swirl_strdup((char *)tokc.str.data);
        next();
        if (tok != ')')
            goto pragma_err;
        if (t == TOK_lib) {
            ...
            if (t == TOK_option)
                swirl_set_options(s1, p);
            swirl_free(p);
        }

    } else if (s1->warn_unsupported) {
        swirl_warning("#pragma %s is ignored", get_tok_str(tok, &tokc));
    }
    return;

pragma_err:
    swirl_error("malformed #pragma directive");
    return;
}
 ...
 redo:
    ...
    case TOK_INCLUDE_NEXT:
	    ...
	    if ((len < 2 || ((buf[0] != '"' || buf[len-1] != '"') &&
			     (buf[0] != '<' || buf[len-1] != '>'))))
	        swirl_error("'#include' expects \"FILENAME\" or <FILENAME>");
	    c = buf[len-1];
        ...
        if (s1->include_stack_ptr >= s1->include_stack + INCLUDE_STACK_SIZE)
            swirl_error("#include recursion too deep");
        ...
        for (; i < n; ++i) {
            ...
            if (i == 0) {
                ...
                pstrncpy(buf1, path, swirl_basename(path) - path);
            ...
            *s1->include_stack_ptr++ = file->prev;
            ...
            swirl_debug_bincl(swirl_state);
            tok_flags |= TOK_FLAG_BOF | TOK_FLAG_BOL;
            ch = file->buf_ptr[0];
            goto the_end;
        }
        swirl_error("include file '%s' not found", buf);
include_done:
        break;
    case TOK_IFNDEF:
        c = 1;
    ...
    do_ifdef:
        next_nomacro();
        if (tok < TOK_IDENT)
            swirl_error("invalid argument for '#if%sdef'", c ? "n" : "");
        if (is_bof) {
            if (c) {
#ifdef INC_DEBUG
    ...
    do_if:
        if (s1->ifdef_stack_ptr >= s1->ifdef_stack + IFDEF_STACK_SIZE)
            swirl_error("memory full (ifdef)");
        *s1->ifdef_stack_ptr++ = c;
        goto test_skip;
    case TOK_ELSE:
        if (s1->ifdef_stack_ptr == s1->ifdef_stack)
            swirl_error("#else without matching #if");
        if (s1->ifdef_stack_ptr[-1] & 2)
            swirl_error("#else after #else");
        c = (s1->ifdef_stack_ptr[-1] ^= 3);
        goto test_else;
    case TOK_ELIF:
        if (s1->ifdef_stack_ptr == s1->ifdef_stack)
            swirl_error("#elif without matching #if");
        c = s1->ifdef_stack_ptr[-1];
        if (c > 1)
            swirl_error("#elif after #else");
    ...
    case TOK_ENDIF:
        if (s1->ifdef_stack_ptr <= file->ifdef_stack_ptr)
            swirl_error("#endif without matching #if");
        s1->ifdef_stack_ptr--;
    ...
    case TOK_WARNING:
        ...
        *q = '\0';
        if (c == TOK_ERROR)
            swirl_error("#error %s", buf);
        else
            swirl_warning("#warning %s", buf);
        break;
    case TOK_PRAGMA:
        pragma_parse(s1);
        break;
    ...
    default:
        ...
        if (tok == '!' && is_bof)
            ...
            goto ignore;
        swirl_warning("Ignoring unknown preprocessing directive #%s", get_tok_str(tok, &tokc));
    ignore:
...
static void parse_escape_string(CString *outstr, const uint8_t *buf, int is_long)
{
    ...
    for(;;) {
        ...
        if (c == '\\') {
            ...
            case 'U': ucn_chars_nr = 8; goto parse_hex_or_ucn;
                ...
                for(unsigned i=1;i<=ucn_chars_nr;i++) {
                    ...
                    if (c >= 'a' && c <= 'f')
                        ...
                        if (ucn_chars_nr!=-1)
                            swirl_error("%u hex digits expected in universal-character-name\n", ucn_chars_nr);
                        break;
                    }
                    n = n * 16 + c;
                    p++;
            ...
            invalid_escape:
                if (c >= '!' && c <= '~')
                    swirl_warning("unknown escape sequence: \'\\%c\'", c);
                else
                    swirl_warning("unknown escape sequence: \'\\x%x\'", c);
                break;
            }
        ...
        invalid_utf8_sequence:
            swirl_warning("ill-formed UTF-8 subsequence starting with: \'\\x%x\'", c);
            c = 0xFFFD;
            p += skip;
            goto add_char_nonext;
...
static void parse_string(const char *s, int len)
{
    ...
    if (sep == '\'') {
        ...
        if (n < 1)
            swirl_error("empty character constant");
        if (n > 1)
            swirl_warning("multi-character character constant");
        for (c = i = 0; i < n; ++i) {
            if (is_long)
    ...
    *q++ = t;
    ...
    if (t == '.') {
        ...
        if (ch == 'x' || ch == 'X') {
            ...
            ch = *p++;
            b = 16;
        } else if (swirl_state->swirl_ext && (ch == 'b' || ch == 'B')) {
            q--;
            ch = *p++;
            b = 2;
        }
    ...
    if (ch == '.' ||
        ...
        *q = '\0';
        ...
        for(;;) {
            ...
            if (t == 'L') {
                if (lcount >= 2)
                    swirl_error("three 'l's in integer constant");
                if (lcount && *(p - 1) != ch)
                    swirl_error("incorrect integer suffix: %s", p1);
                lcount++;
                ch = *p++;
            } else if (t == 'U') {
                if (ucount >= 1)
                    swirl_error("two 'u's in integer constant");
                ucount++;
                ch = *p++;
            } else {
                break;
        ...
        if (ov)
            swirl_warning("integer constant overflow");
...
maybe_newline:
    ...
    default:
        ...
        if (parse_flags & PARSE_FLAG_ASM_FILE)
            goto parse_simple;
        swirl_error("unrecognized character \\x%02x", c);
        break;
    }
    tok_flags = 0;
keep_tok_flags:
...
static int paste_tokens(int t1, CValue *v1, int t2, CValue *v2)
{
    ...
    swirl_open_bf(swirl_state, ":paste:", cstr.size);
    ...
    for (;;) {
        ...
        if (is_space(tok))
            continue;
        swirl_warning("pasting \"%.*s\" and \"%s\" does not give a valid"
    ...
    Sym *s)
{
    ...
    empty_arg:
                ...
                if (!args && !sa && tok == ')')
                    break;
                if (!sa)
                    swirl_error("macro '%s' used with too many args",
                          get_tok_str(s->v, 0));
                tok_str_new(&str);
                parlevel = spc = 0;
            ...
            if (sa) {
                swirl_error("macro '%s' used with too few args",
                      get_tok_str(s->v, 0));
            }
...
static void swirl_predefs(SwirlState *s1, CString *cs, int is_asm)
{
    int a, b, c;
    const char *defs[] = { target_machine_defs, target_os_defs, NULL };
    const char *p;

    sscanf(SWIRL_VERSION, "%d.%d.%d", &a, &b, &c);
    cstr_printf(cs, "#define __SWIRLC__ %d\n", a*10000 + b*100 + c);
    for (a = 0; defs[a]; ++a)
    ...
    if (s1->output_type == SWIRL_OUTPUT_PREPROCESS)
      putdef(cs, "__SWIRL_PP__");
    if (s1->output_type == SWIRL_OUTPUT_MEMORY)
      putdef(cs, "__SWIRL_RUN__");
...
#if CONFIG_SWIRL_PREDEFS
        #include "swirldefs_.h" /* include as strings */
#else
        "#include <swirldefs.h>\n" /* load at runtime */
#endif
        , -1);
...
ST_FUNC void preprocess_start(SwirlState *s1, int filetype)
{
    ...
    if (!(filetype & AFF_TYPE_ASM)) {
        ...
        *s1->include_stack_ptr++ = file;
        swirl_open_bf(s1, "<command line>", cstr.size);
        memcpy(file->buffer, cstr.data, cstr.size);
        cstr_free(&cstr);
    }
...
static void pp_line(SwirlState *s1, BufferedFile *f, int level)
{
...
static void define_print(SwirlState *s1, int v)
{
    FILE *fp;
    Sym *s;
...
static void pp_debug_defines(SwirlState *s1)
{
    int v, t;
    const char *vs;
    FILE *fp;
...
static void pp_debug_builtins(SwirlState *s1)
{
    int v;
