...
ST_FUNC void gsym_addr(int t_, int a_)
{
    ...
    while (t) {
        ...
        if (a - t + 0x8000000 >= 0x10000000)
            swirl_error("branch out of range");
...
static int arm64_type_size(int t)
{
     ...
     * which 'may' help compiler optimizers. See swirl.h
     */
    switch (t & VT_BTYPE) {
...
ST_FUNC void gfunc_call(int nb_args)
{
    ...
    t = swirl_malloc((nb_args + 1) * sizeof(*t));
    a = swirl_malloc((nb_args + 1) * sizeof(*a));
    a1 = swirl_malloc((nb_args + 1) * sizeof(*a1));
    ...
    if (stack >= 0x1000000) // 16Mb
        swirl_error("stack size too big %lu", stack);
    if (stack & 0xfff)
...
ST_FUNC void gfunc_prolog(Sym *func_sym)
{
    ...
    for (sym = func_type->ref; sym; sym = sym->next)
        ++n;
    t = n ? swirl_malloc(n * sizeof(*t)) : NULL;
    a = n ? swirl_malloc(n * sizeof(*a)) : NULL;
                          ...
                          uint32_t x, uint32_t a)
{
    ...
    case TOK_SHR: {
        ...
        if (rev)
            return 0;
        if (!val) {
            // swirl_warning("shift count >= width of type");
