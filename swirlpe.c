/*
 *  swirlpe.c - PE file output for the Swirl compiler
 */
...
#elif defined SWIRL_TARGET_I386
# define ADDR3264 DWORD
# define PE_IMAGE_REL IMAGE_REL_BASED_HIGHLOW
...
static const char *pe_export_name(SwirlState *s1, ElfW(Sym) *sym)
{
...
static int pe_find_import(SwirlState * s1, ElfW(Sym) *sym)
{
    char buffer[200];
    const char *s, *p;
    int sym_index = 0, n = 0;
...
#define CHARACTERISTICS_DLL 0x230F
    ...
    if (NULL == pf.op) {
        swirl_error_noabort("could not write '%s': %s", pe->filename, strerror(errno));
        return -1;
    }
...
static void pe_build_imports(struct pe_info *pe)
{
    ...
    for (i = 0; i < pe->imp_count; ++i) {
        ...
        if (dllindex)
            name = swirl_basename((dllref = pe->s1->loaded_dlls[dllindex-1])->name);
        else
            name = "", dllref = NULL;
        ...
        for (k = 0, n = p->sym_count; k <= n; ++k) {
            if (k < n) {
                ...
                if (dllref)
                    ...
                    ordinal = 0, v = imp_sym->st_value; /* address from swirl_add_symbol() */
                ...
                if (pe->type == PE_RUN) {
                    ...
                    if (!v)
                        swirl_error_noabort("could not resolve symbol '%s'", name);
                } else
#endif
                if (ordinal) {
...
static void pe_build_exports(struct pe_info *pe)
{
    ...
    pe_align_section(pe->thunk, 16);
    dllname = swirl_basename(pe->filename);
    ...
    strcpy(swirl_fileextension(buf), ".def");
    op = fopen(buf, "wb");
    if (NULL == op) {
        swirl_error_noabort("could not create '%s': %s", buf, strerror(errno));
    } else {
...
static int pe_assign_addresses (struct pe_info *pe)
{
    ...
    section_order = swirl_malloc(pe->s1->nb_sections * sizeof (int));
...
static int pe_isafunc(SwirlState *s1, int sym_index)
{
    ...
    for (sym_index = 1; sym_index < sym_end; ++sym_index) {
        ...
        if (sym->st_shndx == SHN_UNDEF) {
            ...
            if (type == STT_FUNC) {
                ...
                /* swirl_realloc might have altered sym's address */
                sym = (ElfW(Sym) *)symtab_section->data + sym_index;
        ...
        not_found:
            if (ELFW(ST_BIND)(sym->st_info) == STB_WEAK)
                ...
                continue;
            swirl_error_noabort("undefined symbol '%s'%s", name,
...
static void pe_print_sections(SwirlState *s1, const char *fname)
{
    Section *s;
    FILE *f;
    int i;
...
static int pe_add_dllref(SwirlState *s1, const char *dllname)
{
    int i;
    for (i = 0; i < s1->nb_loaded_dlls; ++i)
        if (0 == strcmp(s1->loaded_dlls[i]->name, dllname))
            return i + 1;
    swirl_add_dllref(s1, dllname);
...
found:
    ...
    for (i = 0; i < ied.NumberOfNames; ++i) {
        ...
        for (l = 0;;) {
            if (n+1 >= n0)
                p = swirl_realloc(p, n0 = n0 ? n0 * 2 : 256);
...
static int pe_load_res(SwirlState *s1, int fd)
{
...
/* ------------------------------------------------------------- */
static int pe_load_def(SwirlState *s1, int fd)
{
...
/* ------------------------------------------------------------- */
static int pe_load_dll(SwirlState *s1, const char *filename)
{
    char *p, *q;
    int index, ret;

    ret = swirl_get_dllexports(filename, &p);
    if (ret) {
        return -1;
    } else if (p) {
...
ST_FUNC int pe_load_file(struct SwirlState *s1, int fd, const char *filename)
{
    int ret = -1;
    char buf[10];
    if (0 == strcmp(swirl_fileextension(filename), ".def"))
...
/* ------------------------------------------------------------- */
#ifdef SWIRL_TARGET_X86_64
static unsigned pe_add_uwwind_info(SwirlState *s1)
{
    if (NULL == s1->uw_pdata) {
...
{
    if (swirl_add_dll(s1, filename, 0) < 0)
        swirl_error_noabort("%s not found", filename);
}

static void pe_add_runtime(SwirlState *s1, struct pe_info *pe)
{
    const char *start_symbol;
    int pe_type;

    if (SWIRL_OUTPUT_DLL == s1->output_type) {
        pe_type = PE_DLL;
        ...
        if (SWIRL_OUTPUT_MEMORY == s1->output_type)
    ...
    /* grab the startup code from libswirl1.a */
#ifdef SWIRL_IS_NATIVE
    if (SWIRL_OUTPUT_MEMORY != s1->output_type || s1->runtime_main)
#endif
    ...
    if (0 == s1->nostdlib) {
        ...
        for (pp = libs; 0 != (p = *pp); ++pp) {
            if (*p)
                swirl_add_library_err(s1, p);
            else if (PE_DLL != pe_type && PE_GUI != pe_type)
                break;
        }
    }

    /* need this for 'swirlelf.c:relocate_section()' */
    if (SWIRL_OUTPUT_DLL == s1->output_type)
...
static void pe_set_options(SwirlState * s1, struct pe_info *pe)
{
    if (PE_DLL == pe->type) {
...
#ifdef PE_PRINT_SECTIONS
    pe_print_sections(s1, "swirl.log");
#endif
    return ret;
}
