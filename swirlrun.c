/*
 *  Swirl compiler - Support for -run switch
 */
...
{
    /* --> swirlelf.c:swirl_add_btstub wants those below in that order: */
...
#endif /* CONFIG_SWIRL_BACKTRACE */

/* defined when included from lib/bt-exe.c */
#ifndef CONFIG_SWIRL_BACKTRACE_ONLY

#ifndef _WIN32
# include <sys/mman.h>
#endif

static void set_pages_executable(SwirlState *s1, void *ptr, unsigned long length);
static int swirl_relocate_ex(SwirlState *s1, void *ptr, addr_t ptr_diff);

#ifdef _WIN64
static void *win64_add_function_table(SwirlState *s1);
...
LIBSWIRLAPI int swirl_relocate(SwirlState *s1, void *ptr)
{
    ...
    if (SWIRL_RELOCATE_AUTO != ptr)
        return swirl_relocate_ex(s1, ptr, 0);

    size = swirl_relocate_ex(s1, NULL, 0);
    if (size < 0)
        return -1;
...
#ifdef HAVE_SELINUX
{
    ...
    void *prx;
    char tmpfname[] = "/tmp/.swirlrunXXXXXX";
	...
	swirl_error("swirlrun: could not map memory");
...
#endif
    swirl_relocate_ex(s1, ptr, ptr_diff); /* no more errors expected */
...
#endif
        swirl_free(s1->runtime_mem[i]);
#endif
    }
    swirl_free(s1->runtime_mem);
}

static void run_cdtors(SwirlState *s1, const char *start, const char *end,
...
LIBSWIRLAPI int swirl_run(SwirlState *s1, int argc, char **argv)
{
...
#ifdef CONFIG_SWIRL_BACKTRACE
    if (s1->do_debug)
        swirl_add_symbol(s1, "exit", rt_exit);
#endif
    if (swirl_relocate(s1, SWIRL_RELOCATE_AUTO) < 0)
        return -1;
...
#endif
        rc->top_func = swirl_get_symbol(s1, "main");
        rc->num_callers = s1->rt_num_callers;
        rc->do_jmp = 1;
        if ((p = swirl_get_symbol(s1, "__rt_error")))
            *(void**)p = _rt_error;
#ifdef CONFIG_SWIRL_BCHECK
        if (s1->do_bounds_check) {
            if ((p = swirl_get_symbol(s1, "__bound_init")))
...
# ifndef HAVE_SELINUX
    ...
    if (mprotect((void *)start, end - start, PROT_READ | PROT_WRITE | PROT_EXEC))
        swirl_error("mprotect failed: did you mean to configure --with-selinux?");
# endif
/* XXX: BSD sometimes dump core with bad system call */
# if (defined(SWIRL_TARGET_ARM) && \
      !defined(__FreeBSD__) && !defined(__OpenBSD__) && !defined(__NetBSD__)) || \
     defined(SWIRL_TARGET_ARM64)
...
#endif
#endif //ndef CONFIG_SWIRL_BACKTRACE_ONLY
/* ------------------------------------------------------------- */
#ifdef CONFIG_SWIRL_BACKTRACE
...
static int _rt_error(void *fp, void *ip, const char *fmt, va_list ap)
{
    ...
    if (fp) {
        /* we're called from swirl_backtrace. */
        rc->fp = (addr_t)fp;
        rc->ip = (addr_t)ip;
        msg = "";
    } else {
...
static void set_exception_handler(void)
{
    ...
    /* install SWIRL signal handlers to print debug info on fatal
       runtime errors */
...
static SWIRLSyms swirl_syms[] = {
#if !defined(CONFIG_SWIRLBOOT)
#define SWIRLSYM(a) { #a, &a, },
    SWIRLSYM(printf)
    SWIRLSYM(fprintf)
    SWIRLSYM(fopen)
    SWIRLSYM(fclose)
...
#endif /* CONFIG_SWIRL_STATIC */
#endif /* SWIRL_IS_NATIVE */
/* ------------------------------------------------------------- */
