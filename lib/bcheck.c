/* Swirl memory and bounds checker */
#include <stdlib.h>
#include <stdio.h>
#include <stdarg.h>
#include <string.h>
...
#define SWIRL_TYPE_NONE           (0)
#define SWIRL_TYPE_MALLOC         (1)
#define SWIRL_TYPE_CALLOC         (2)
#define SWIRL_TYPE_REALLOC        (3)
#define SWIRL_TYPE_MEMALIGN       (4)
#define SWIRL_TYPE_STRDUP         (5)
...
int swirl_backtrace(const char *fmt, ...);

/* print a bound error message */
#define bound_warning(...) \
    swirl_backtrace("^bcheck.c^BCHECK: " __VA_ARGS__)
...
void __bound_init(size_t *p, int mode)
{
    ...
    print_warn_ptr_add = getenv ("SWIRL_BOUNDS_WARN_POINTER_ADD") != NULL;
    print_calls = getenv ("SWIRL_BOUNDS_PRINT_CALLS") != NULL;
    print_heap = getenv ("SWIRL_BOUNDS_PRINT_HEAP") != NULL;
    print_statistic = getenv ("SWIRL_BOUNDS_PRINT_STATISTIC") != NULL;
    never_fatal = getenv ("SWIRL_BOUNDS_NEVER_FATAL") != NULL;
...
#if MALLOC_REDIR
    {
        ...
        /* swirl -run required RTLD_DEFAULT. normal usage requires RTLD_NEXT,
...
#ifdef __linux__
    {
        ...
        /* display exec name. useful when a lot of code is compiled with swirl */
...
#else
    /* does not work if locale is changed */
...
void __attribute__((destructor)) __bound_exit(void)
{
    ...
    if (inited) {
#if !defined(_WIN32) && !defined(__APPLE__) && !defined SWIRL_MUSL && \
...
#endif
    /* allocate one more byte to ensure the regions will be
...
#endif
{
    ...
    if (align > 4) {
        ...
        /* suppose that malloc aligns to at least four bytes */
        ptr = BOUND_MALLOC(size + 1);
    }
#endif
    dprintf(stderr, "%s, %s(): %p, 0x%lx\n",
