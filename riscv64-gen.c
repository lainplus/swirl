...
{
    assert(r >= 8 && r < 16);
    return r - 8 + 10;  // swirlfX --> faX == f(10+X)
}
...
ST_FUNC void gsym_addr(int t_, int a_)
{
    ...
    while (t) {
        ...
        uint32_t r = a - t, imm;
        if ((r + (1 << 21)) & ~((1U << 22) - 2))
          swirl_error("out-of-range branch chain");
        imm =   (((r >> 12) &  0xff) << 12)
            | (((r >> 11) &     1) << 20)
            | (((r >>  1) & 0x3ff) << 21)
            | (((r >> 20) &     1) << 31);
    ...
    if (sv->r & VT_SYM) {
        ...
        if (sv->sym->type.t & VT_STATIC) { // XXX do this per linker relax
            ...
            if (((unsigned)fc + (1 << 11)) >> 12)
              swirl_error("unimp: large addend for global address (0x%lx)", (long)sv->c.i);
            greloca(cur_text_section, sv->sym, ind,
                    R_RISCV_GOT_HI20, 0);
            doload = 1;
        }
        ...
        rr = 8; // s0
        if (fc != sv->c.i)
          swirl_error("unimp: store(giant local off) (0x%lx)", (long)sv->c.i);
        if (((unsigned)fc + (1 << 11)) >> 12) {
...
ST_FUNC void load(int r, SValue *sv)
{
    ...
    if (fr & VT_LVAL) {
        ...
        if (v == VT_LOCAL || (fr & VT_SYM)) {
            ...
            br = ireg(v);
            /*if (((unsigned)fc + (1 << 11)) >> 12)
              swirl_error("unimp: load(large addend) (0x%x)", fc);*/
		...
		load_large_constant(rr, fc, si);
            ...
            swirl_error("unimp: load(non-local lval)");
        }
      ...
      swirl_error("unimp: load(non-const)");
}
...
ST_FUNC void store(int r, SValue *sv)
{
    ...
    if (bt == VT_LDOUBLE)
      size = align = 8;
    if (bt == VT_STRUCT)
      swirl_error("unimp: store(struct)");
    if (size > 8)
      swirl_error("unimp: large sized store");
    assert(sv->r & VT_LVAL);
    if (fr == VT_LOCAL || (sv->r & VT_SYM)) {
        ...
        ptrreg = ireg(fr);
        /*if (((unsigned)fc + (1 << 11)) >> 12)
          swirl_error("unimp: store(large addend) (0x%x)", fc);*/
      ...
      swirl_error("implement me: %s(!local)", __FUNCTION__);
...
ST_FUNC void gfunc_call(int nb_args)
{
    ...
    int *info = swirl_malloc((nb_args + 1) * sizeof (int));
    int stack_adj = 0, tempspace = 0, stack_add, ofs, splitofs = 0;
    SValue *sv;
    Sym *sa;

#ifdef CONFIG_SWIRL_BCHECK
    int bc_save = swirl_state->do_bounds_check;
    ...
    for (i = 0; i < nb_args; i++) {
        ...
        sv = &vtop[1 + i - nb_args];
        sv->type.t &= ~VT_ARRAY; // XXX this should be done in swirlgen.c
        size = type_size(&sv->type, &align);
        if (size > 16) {
            if (align < XLEN)
              align = XLEN;
    ...
    for (i = 0; i < nb_args; i++) {
		...
		if ((origtype.t & VT_BTYPE) == VT_STRUCT)
                    swirl_state->do_bounds_check = 0;
#endif
                gen_op('+');
#ifdef CONFIG_SWIRL_BCHECK
		swirl_state->do_bounds_check = bc_save;
#endif
                indir();
                vtop->type = origtype;
                loadt = vtop->type.t & VT_BTYPE;
...
static void gen_opil(int op, int ll)
{
    ...
    default:
        ...
        swirl_error("implement me: %s(%s)", __FUNCTION__, get_tok_str(op, NULL));
        break;
