/*
 *  i386 specific functions for Swirl assembler
 */
...
static inline int get_reg_shift(SwirlState *s1)
{
    int shift, v;
    v = asm_int_expr(s1);
    switch(v) {
...
static void parse_operand(SwirlState *s1, Operand *op)
{
    ...
    if (tok == '%') {
        ...
        reg_error:
            swirl_error("unknown register %%%s", get_tok_str(tok, &tokc));
        }
        next();
    no_skip: ;
    } else if (tok == '$') {
...
static void gen_disp32(ExprValue *pe)
{
    ...
    if (esym && esym->st_shndx == cur_text_section->sh_num) {
        /* same section: we can output an absolute value. Note
           that the SWIRL compiler behaves differently here because
		    ...
		    int regi, int rmi)
{
  ...
  if (rex) {
      if (saw_high_8bit)
	  swirl_error("can't encode register %%%ch when REX prefix is required",
...
ST_FUNC void asm_opcode(SwirlState *s1, int opcode)
{
    ...
    for(;;) {
        ...
        if (nb_ops >= MAX_OPERANDS) {
            swirl_error("incorrect number of operands");
        }
        parse_operand(s1, pop);
        if (tok == ':') {
           if (pop->type != OP_SEG || seg_prefix)
               swirl_error("incorrect prefix");
           seg_prefix = segment_prefixes[pop->reg];
           next();
           parse_operand(s1, pop);
           if (!(pop->type & OP_EA)) {
               swirl_error("segment prefix must be followed by memory reference");
           }
        }
        pop++;
        nb_ops++;
...
again:
    ...
    if (pa->sym == 0) {
        if (opcode >= TOK_ASM_first && opcode <= TOK_ASM_last) {
            ...
            swirl_error("bad operand with opcode '%s'",
		...
		goto again;
            ...
            swirl_error("unknown opcode '%s'", ts->str);
        }
    }
	   ...
	   are such opcodes.  */
        ...
        if (s == autosize) {
	        ...
	        s = NBWLX - 2;
                ...
                swirl_error("cannot infer opcode suffix");
        }
    }
	...
	esym = elfsym(ops[0].e.sym);
	    ...
	    if (v == 0xeb) /* jmp */
	        ...
	        swirl_error("invalid displacement");
        }
    }
                                    ...
                                    int *pout_reg)
{
	...
	if (op->reg >= 0) {
	    if (is_reg_allocated(op->reg))
	        swirl_error("asm regvar requests register that's taken already");
	    reg = op->reg;
	    goto reg_found;
	}
    try_next:
        ...
        case '&':
            if (j >= nb_outputs)
                swirl_error("'%c' modifier can only be applied to outputs", c);
            reg_mask = REG_IN_MASK | REG_OUT_MASK;
            goto try_next;
        case 'A':
                              ...
                              SValue *sv, int modifier)
{
    ...
    no_offset:;
        ...
        if (modifier == 'b') {
            if (reg >= 4)
                swirl_error("cannot use byte register");
            size = 1;
        } else if (modifier == 'h') {
            if (reg >= 4)
                swirl_error("cannot use byte register");
