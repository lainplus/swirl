/*
 *  x86-64 code generator for Swirl
 */
...
/* define if return values need to be extended explicitely
   at caller side (for interfacing with non-SWIRL compilers) */
#define PROMOTE_RET
/******************************************************/
...
{
#ifdef SWIRL_TARGET_PE
    swirl_error("internal error: no GOT on PE: %s %x %x | %02x %02x %02x\n",
...
void load(int r, SValue *sv)
{
    ...
    if (fc != sv->c.i && (fr & VT_SYM))
      swirl_error("64 bit addend in load");
	...
	/* Like GCC we can load from small enough properly sized
	   ...
	   occur only with asm, so swirlasm.c might also be a better place */
	if ((ft & VT_BTYPE) == VT_STRUCT) {
		...
		default:
		    swirl_error("invalid aggregate type for register load");
		    break;
	    }
	}
...
void store(int r, SValue *v)
{
    ...
    if (fc != v->c.i && (fr & VT_SYM))
      swirl_error("64 bit addend in store");
    ft &= ~(VT_VOLATILE | VT_CONSTANT);
    bt = ft & VT_BTYPE;
...
void gfunc_call(int nb_args)
{
    ...
    for(i = 0; i < nb_args; i++) {
		...
		if (swirl_state->nosse)
		  swirl_error("SSE disabled");
                if (arg >= REGN) {
                    gv(RC_XMM0);
                    /* movq %xmm0, j*8(%rsp) */
...
void gfunc_call(int nb_args)
{
    ...
    int nb_sse_args = 0;
    int sse_reg, gen_reg;
    char *onstack = swirl_malloc((nb_args + 1) * sizeof (char));
    ...
    if (nb_sse_args && swirl_state->nosse)
      swirl_error("SSE disabled but floating point arguments passed");
...
void gfunc_prolog(Sym *func_sym)
{
    ...
    while ((sym = sym->next) != NULL) {
        ...
        case x86_64_mode_sse:
	    if (swirl_state->nosse)
	        swirl_error("SSE disabled but floating point arguments used");
...
#if defined(CONFIG_SWIRL_BCHECK)
    use_call = swirl_state->do_bounds_check;
#endif
#ifdef SWIRL_TARGET_PE	/* alloca does more than just adjust %rsp on Windows */
    use_call = 1;
#endif
    if (use_call)
    {
