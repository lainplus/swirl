...
ST_FUNC void swirlelf_stab_new(SwirlState *s)
{
    ...
    if (s->do_backtrace && s->output_type != SWIRL_OUTPUT_MEMORY)
...
ST_FUNC void swirlelf_delete(SwirlState *s1)
{
    int i;
    ...
    for (i = 0; i < nb_sym_versions; i++) {
        swirl_free(sym_versions[i].version);
        swirl_free(sym_versions[i].lib);
    }
    swirl_free(sym_versions);
    swirl_free(sym_to_version);
#endif
...
# endif
    ...
    swirl_free(s1->sym_attrs);

    symtab_section = NULL; /* for swirlrun.c:rt_printline() */
}
...
ST_FUNC void swirlelf_end_file(SwirlState *s1)
{
    ...
    tr = swirl_mallocz(nb_syms * sizeof *tr);
    ...
    for (i = 1; i < s1->nb_sections; i++) {
        ...
        if (sr->sh_type == SHT_RELX && sr->link == s) {
            ...
            for (; rel < rel_end; ++rel) {
                ...
                //if (n < 0) swirl_error("internal: invalid symbol index in relocation");
...
ST_FUNC Section *new_section(SwirlState *s1, const char *name, int sh_type, int sh_flags)
{
    ...
    sec = swirl_mallocz(sizeof(Section) + strlen(name));
    sec->s1 = s1;
...
ST_FUNC Section *new_symtab(SwirlState *s1,
...
ST_FUNC void section_realloc(Section *sec, unsigned long new_size)
{
    ...
    while (size < new_size)
        size = size * 2;
    data = swirl_realloc(sec->data, size);
...
static Section *find_section_create (SwirlState *s1, const char *name, int create)
{
    Section *sec;
    int i;
    for(i = 1; i < s1->nb_sections; i++) {
...
/* return elf symbol value, signal error if 'err' is nonzero, decorate
   name if FORC */
ST_FUNC addr_t get_sym_addr(SwirlState *s1, const char *name, int err, int forc)
{
    int sym_index;
    ElfW(Sym) *sym;
    char buf[256];
    ...
    if (!sym_index || sym->st_shndx == SHN_UNDEF) {
        if (err)
            swirl_error("%s not defined", name);
        return (addr_t)-1;
    }
    return sym->st_value;
}
...
#endif
                swirl_error_noabort("'%s' defined twice", name);
            }
        } else {
        do_patch:
                            ...
                            int type, int symbol, addr_t addend)
{
    ...
    if (SHT_RELX != SHT_RELA && addend)
        swirl_error("non-zero addend on REL architecture");
}
...
ST_FUNC void put_stabs(SwirlState *s1, const char *str, int type, int other, int desc,
                      unsigned long value)
{
    Stab_Sym *sym;
...
ST_FUNC void put_stabs_r(SwirlState *s1, const char *str, int type, int other, int desc,
...
ST_FUNC struct sym_attr *get_sym_attr(SwirlState *s1, int index, int alloc)
{
    ...
    if (index >= s1->nb_sym_attrs) {
        ...
        while (index >= n)
            n *= 2;
        tab = swirl_realloc(s1->sym_attrs, n * sizeof(*s1->sym_attrs));
        s1->sym_attrs = tab;
...
/* In an ELF file symbol table, the local symbols must appear below
   the global and weak ones. Since SWIRL cannot sort it while generating
   the code, we must do it after. All the relocation tables are also
   modified to take into account the symbol table sorting */
static void sort_syms(SwirlState *s1, Section *s)
{
    int *old_to_new_syms;
    ElfW(Sym) *new_syms;
    int nb_syms, i;
    ...
    nb_syms = s->data_offset / sizeof(ElfW(Sym));
    new_syms = swirl_malloc(nb_syms * sizeof(ElfW(Sym)));
    old_to_new_syms = swirl_malloc(nb_syms * sizeof(int));
...
ST_FUNC void relocate_syms(SwirlState *s1, Section *symtab, int do_resolve)
{
    ...
    for_each_elem(symtab, 1, sym, ElfW(Sym)) {
        ...
        if (sh_num == SHN_UNDEF) {
            ...
            /* Use ld.so to resolve symbol for us (for swirl -run) */
            if (do_resolve) {
#if defined SWIRL_IS_NATIVE && !defined SWIRL_TARGET_PE
		...
		if (addr == NULL) {
            ...
            if (sym_bind == STB_WEAK)
                ...
                swirl_error_noabort("undefined symbol '%s'", name);
...
ST_FUNC void relocate_section(SwirlState *s1, Section *s)
{
    ...
    if (sr->sh_flags & SHF_ALLOC) {
        sr->link = s1->dynsym;
        if (s1->output_type == SWIRL_OUTPUT_DLL) {
...
static void relocate_rel(SwirlState *s1, Section *sr)
{
    Section *s;
    ElfW_Rel *rel;
...
/* count the number of dynamic relocations so that we can reserve
   their space */
static int prepare_dynamic_rel(SwirlState *s1, Section *sr)
{
    int count = 0;
#if defined(SWIRL_TARGET_I386) || defined(SWIRL_TARGET_X86_64) || \
    defined(SWIRL_TARGET_ARM) || defined(SWIRL_TARGET_ARM64) || \
    defined(SWIRL_TARGET_RISCV64)
    ElfW_Rel *rel;
    for_each_elem(sr, 0, rel, ElfW_Rel) {
...
static void build_got(SwirlState *s1)
{
...
static struct sym_attr * put_got_entry(SwirlState *s1, int dyn_reloc_type,
                                       int sym_index)
{
    int need_plt_entry;
    const char *name;
...
static void build_got_entries_pass(SwirlState *s1, int pass)
{
    Section *s;
    ElfW_Rel *rel;
    ElfW(Sym) *sym;
    ...
    for(i = 1; i < s1->nb_sections; i++) {
        ...
        for_each_elem(s, 0, rel, ElfW_Rel) {
            ...
            if (gotplt_entry == -1)
                swirl_error ("Unknown relocation type for got: %d", type);
            sym_index = ELFW(R_SYM)(rel->r_info);
            sym = &((ElfW(Sym) *)symtab_section->data)[sym_index];
	       ...
	       probably created by swirl_add_symbol, and thus on 64-bit
	       targets might be too far from application code.  */
            if (gotplt_entry == AUTO_GOTPLT_ENTRY) {
                if (sym->st_shndx == SHN_UNDEF) {
                    ElfW(Sym) *esym;
		    int dynindex;
                    if (s1->output_type == SWIRL_OUTPUT_DLL && ! PCRELATIVE_DLLPLT)
                        continue;
		    /* Relocations for UNDEF symbols would normally need
		       ...
		       But SWIRL doesn't do that (at least for exes), so we
		...
		sym->st_shndx != SHN_UNDEF &&
		 ...
		 ELFW(ST_BIND)(sym->st_info) == STB_LOCAL ||
		 s1->output_type == SWIRL_OUTPUT_EXE)) {
		if (pass == 0)
		    continue;
            ...
            if (reloc_type == -1)
                swirl_error ("Unknown relocation type: %d", type);
...
static void add_init_array_defines(SwirlState *s1, const char *section_name)
{
    Section *s;
    addr_t end_offset;
    char buf[1024];
...
static void swirl_add_support(SwirlState *s1, const char *filename)
{
    char buf[1024];
    snprintf(buf, sizeof(buf), "%s/%s", s1->swirl_lib_path, filename);
    swirl_add_file(s1, buf);
}
#endif
...
#ifdef CONFIG_SWIRL_BACKTRACE
static void put_ptr(SwirlState *s1, Section *s, int offs)
{
    int c;
...
static void set_local_sym(SwirlState *s1, const char *name, Section *s, int offset)
{
...
ST_FUNC void swirl_add_btstub(SwirlState *s1)
{
    ...
    o = s->data_offset;
    /* create (part of) a struct rt_context (see swirlrun.c) */
    ...
    cstr_printf(&cstr, "__bt_init(__rt_info,%d, 0);}",
        s1->output_type == SWIRL_OUTPUT_DLL ? 0 : s1->rt_num_callers + 1);
    swirl_compile_string(s1, cstr.data);
    cstr_free(&cstr);
...
ST_FUNC void swirl_add_runtime(SwirlState *s1)
{
    ...
    swirl_add_bcheck(s1);
#endif
    swirl_add_pragma_libs(s1);
    /* add libc */
    if (!s1->nostdlib) {
        if (s1->option_pthread)
            swirl_add_library_err(s1, "pthread");
        swirl_add_library_err(s1, "c");
#ifdef SWIRL_LIBGCC
        if (!s1->static_link) {
            if (SWIRL_LIBGCC[0] == '/')
                swirl_add_file(s1, SWIRL_LIBGCC);
            else
                swirl_add_dll(s1, SWIRL_LIBGCC, 0);
        }
#endif
#if SWIRL_TARGET_ARM && TARGETOS_FreeBSD
        swirl_add_library_err(s1, "gcc_s"); // unwind code
#endif
#ifdef CONFIG_SWIRL_BCHECK
        if (s1->do_bounds_check && s1->output_type != SWIRL_OUTPUT_DLL) {
            swirl_add_library_err(s1, "pthread");
#if !TARGETOS_OpenBSD && !TARGETOS_NetBSD
            swirl_add_library_err(s1, "dl");
#endif
            swirl_add_support(s1, "bcheck.o");
	    if (s1->static_link)
                swirl_add_library_err(s1, "c");
        }
#endif
#ifdef CONFIG_SWIRL_BACKTRACE
        if (s1->do_backtrace) {
            if (s1->output_type == SWIRL_OUTPUT_EXE)
                swirl_add_support(s1, "bt-exe.o");
            if (s1->output_type != SWIRL_OUTPUT_DLL)
                swirl_add_support(s1, "bt-log.o");
            if (s1->output_type != SWIRL_OUTPUT_MEMORY)
                swirl_add_btstub(s1);
        }
#endif
        if (strlen(SWIRL_LIBSWIRL1) > 0)
            swirl_add_support(s1, SWIRL_LIBSWIRL1);
#if TARGETOS_OpenBSD || TARGETOS_FreeBSD || TARGETOS_NetBSD
        /* add crt end if not memory output */
	if (s1->output_type != SWIRL_OUTPUT_MEMORY) {
	    if (s1->output_type == SWIRL_OUTPUT_DLL)
	        swirl_add_crt(s1, "crtendS.o");
	    else
	        swirl_add_crt(s1, "crtend.o");
#if TARGETOS_FreeBSD || TARGETOS_NetBSD
            swirl_add_crt(s1, "crtn.o");
#endif
        }
...
/* add various standard linker symbols (must be done after the
   ...
   symbols)) */
static void swirl_add_linker_symbols(SwirlState *s1)
{
    char buf[1024];
    int i;
    Section *s;
...
ST_FUNC void resolve_common_syms(SwirlState *s1)
{
    ...
    swirl_add_linker_symbols(s1);
}

#ifndef ELF_OBJ_ONLY

ST_FUNC void fill_got_entry(SwirlState *s1, ElfW_Rel *rel)
{
    int sym_index = ELFW(R_SYM) (rel->r_info);
    ElfW(Sym) *sym = &((ElfW(Sym) *) symtab_section->data)[sym_index];
...
ST_FUNC void fill_got(SwirlState *s1)
{
    Section *s;
    ElfW_Rel *rel;
    int i;
...
static void fill_local_got_entries(SwirlState *s1)
{
    ElfW_Rel *rel;
    if (!s1->got->reloc)
        return;
...
static void bind_exe_dynsyms(SwirlState *s1)
{
    const char *name;
    int sym_index, index;
    ElfW(Sym) *sym, *esym;
...
static void bind_libs_dynsyms(SwirlState *s1)
{
    const char *name;
    int sym_index;
    ElfW(Sym) *sym, *esym;
    ...
    for_each_elem(s1->dynsymtab_section, 1, esym, ElfW(Sym)) {
        ...
        if (sym_index && sym->st_shndx != SHN_UNDEF
            ...
            if (ELFW(ST_BIND)(esym->st_info) != STB_WEAK)
                swirl_warning("undefined dynamic symbol '%s'", name);
        }
    }
}
...
static void export_global_syms(SwirlState *s1)
{
    int dynindex, index;
    const char *name;
    ElfW(Sym) *sym;
...
static int set_sec_sizes(SwirlState *s1)
{
    int i;
    Section *s;
    int textrel = 0;
...
static void alloc_sec_names(
    SwirlState *s1, int is_obj
    );

static int layout_any_sections(
    SwirlState *s1, int file_offset, int *sec_order, int is_obj
    );

/* Assign sections to segments and decide how are sections laid out when loaded
   in memory. This function also fills corresponding program headers. */
static int layout_sections(SwirlState *s1, ElfW(Phdr) *phdr,
                           ...
                           struct ro_inf *roinf, int *sec_order)
{
    ...
    file_offset = 0;
    if (s1->output_format == SWIRL_OUTPUT_FORMAT_ELF)
...
/* Fill the dynamic section with tags describing the address and size of
   sections */
static void fill_dynamic(SwirlState *s1, struct dyn_inf *dyninf)
{
...
/* Relocate remaining sections and symbols (that is those not related to
   dynamic linking) */
static int final_sections_reloc(SwirlState *s1)
{
    int i;
    Section *s;
...
/* Remove gaps between RELX sections.
   ...
   The gaps are then filled with 0 in swirl_output_elf. The 0 is intepreted as
...
static void update_reloc_sections(SwirlState *s1, struct dyn_inf *dyninf)
{
    int i;
...
static void swirl_output_elf(SwirlState *s1, FILE *f, int phnum, ElfW(Phdr) *phdr,
                           int file_offset, int *sec_order)
{
    ...
    if (file_type == SWIRL_OUTPUT_EXE || file_type == SWIRL_OUTPUT_DLL)
    ...
    switch(file_type) {
    default:
    case SWIRL_OUTPUT_EXE:
        ehdr.e_type = ET_EXEC;
        ehdr.e_entry = get_sym_addr(s1, "_start", 1, 0);
        break;
    case SWIRL_OUTPUT_DLL:
        ehdr.e_type = ET_DYN;
        ehdr.e_entry = text_section->sh_addr; /* XXX: is it correct ? */
        break;
    case SWIRL_OUTPUT_OBJ:
        ehdr.e_type = ET_REL;
        break;
    }
    ehdr.e_machine = EM_SWIRL_TARGET;
...
static void swirl_output_binary(SwirlState *s1, FILE *f,
                              const int *sec_order)
{
    Section *s;
    int i, offset, size;
...
static int swirl_write_elf_file(SwirlState *s1, const char *filename, int phnum,
                              ElfW(Phdr) *phdr, int file_offset, int *sec_order)
{
    ...
    if (fd < 0) {
        swirl_error_noabort("could not write '%s'", filename);
        return -1;
    }
    f = fdopen(fd, "wb");
    if (s1->verbose)
    ...
    if (s1->output_format == SWIRL_OUTPUT_FORMAT_COFF)
        swirl_output_coff(s1, f);
    else
#endif
    if (s1->output_format == SWIRL_OUTPUT_FORMAT_ELF)
        swirl_output_elf(s1, f, phnum, phdr, file_offset, sec_order);
    else
        swirl_output_binary(s1, f, sec_order);
    fclose(f);
...
static void tidy_section_headers(SwirlState *s1, int *sec_order)
{
    int i, nnew, l, *backmap;
    Section **snew, *s;
    ElfW(Sym) *sym;

    snew = swirl_malloc(s1->nb_sections * sizeof(snew[0]));
    backmap = swirl_malloc(s1->nb_sections * sizeof(backmap[0]));
...
#ifdef SWIRL_TARGET_ARM
static void create_arm_attribute_section(SwirlState *s1)
{
...
#if TARGETOS_OpenBSD || TARGETOS_NetBSD
static Section *create_bsd_note_section(SwirlState *s1,
					const char *name,
					const char *value)
{
...
static int elf_output_file(SwirlState *s1, const char *filename)
{
...

    {
        ...
        swirl_add_runtime(s1);
	resolve_common_syms(s1);

        if (!s1->static_link) {
            if (file_type == SWIRL_OUTPUT_EXE) {
                char *ptr;
    ...
    phdr = swirl_mallocz(phnum * sizeof(ElfW(Phdr)));
    /* compute number of sections */
    shnum = s1->nb_sections;
    /* this array is used to reorder sections in the output file */
    sec_order = swirl_malloc(sizeof(int) * shnum);
    sec_order[0] = 0;
       ...
       linking. */
    {
        ...
        if (dynamic) {
            ...
            if (file_type == SWIRL_OUTPUT_EXE
                || (RELOCATE_DLLPLT && file_type == SWIRL_OUTPUT_DLL))
                relocate_plt(s1);
	...
	tidy_section_headers(s1, sec_order);
        ...
        /* Perform relocation to GOT or PLT entries */
        if (file_type == SWIRL_OUTPUT_EXE && s1->static_link)
            fill_got(s1);
        else if (s1->got)
            fill_local_got_entries(s1);
    }
    /* Create the ELF file with name 'filename' */
    ret = swirl_write_elf_file(s1, filename, phnum, phdr, file_offset, sec_order);
    s1->nb_sections = shnum;

 the_end:
    swirl_free(sec_order);
    swirl_free(phdr);
    return ret;
}
#endif /* ndef ELF_OBJ_ONLY */
...
/* Allocate strings for section names */
static void alloc_sec_names(SwirlState *s1, int is_obj)
{
    int i;
    Section *s, *strsec;
...
static int layout_any_sections(SwirlState *s1, int file_offset, int *sec_order, int is_obj)
{
    int i;
    Section *s;
    for(i = 1; i < s1->nb_sections; i++) {
...
static int elf_output_obj(SwirlState *s1, const char *filename)
{
    int ret, file_offset;
    int *sec_order;
    s1->nb_errors = 0;
    ...
    /* this array is used to reorder sections in the output file */
    sec_order = swirl_malloc(sizeof(int) * s1->nb_sections);
    sec_order[0] = 0;
    file_offset = layout_any_sections(s1, sizeof (ElfW(Ehdr)), sec_order + 1, 1);

    /* Create the ELF file with name 'filename' */
    ret = swirl_write_elf_file(s1, filename, 0, NULL, file_offset, sec_order);
    swirl_free(sec_order);
    return ret;
}
...
{
    if (s->output_type == SWIRL_OUTPUT_OBJ)
...
ST_FUNC int swirl_load_object_file(SwirlState *s1,
                                int fd, unsigned long file_offset)
{
    ...
    lseek(fd, file_offset, SEEK_SET);
    if (swirl_object_type(fd, &ehdr) != AFF_BINTYPE_REL)
        goto fail1;
    /* test CPU specific stuff */
    if (ehdr.e_ident[5] != ELFDATA2LSB ||
        ehdr.e_machine != EM_SWIRL_TARGET) {
    fail1:
        swirl_error_noabort("invalid object file");
        return -1;
    }
    /* read sections */
    shdr = load_data(fd, file_offset + ehdr.e_shoff,
                     sizeof(ElfW(Shdr)) * ehdr.e_shnum);
    sm_table = swirl_mallocz(sizeof(SectionMergeInfo) * ehdr.e_shnum);
    ...
    for(i = 1; i < ehdr.e_shnum; i++) {
        ...
        if (sh->sh_type == SHT_SYMTAB) {
            if (symtab) {
                swirl_error_noabort("object must contain only one symtab");
            fail:
                ret = -1;
                goto the_end;
            }
...
#endif
            {
                swirl_error_noabort("invalid section type");
                goto fail;
	    }
        }
    ...
    /* resolve symbols */
    old_to_new_syms = swirl_mallocz(nb_syms * sizeof(int));
    ...
    for(i = 1; i < ehdr.e_shnum; i++) {
	    ...
	    for (rel = (ElfW_Rel *) s->data + (offset / sizeof(*rel));
		 ...
		 rel++) {
                ...
                invalid_reloc:
                    swirl_error_noabort("Invalid relocation entry [%2d] '%s' @ %.8x",
...
static int swirl_load_alacarte(SwirlState *s1, int fd, int size, int entrysize)
{
    ...
    do {
        ...
        for (p = ar_names, i = 0; i < nsyms; i++, p += strlen(p)+1) {
            ...
            if (len <= 0 || memcmp(hdr.ar_fmag, ARFMAG, 2)) {
                swirl_error_noabort("invalid archive");
                goto the_end;
            }
            off += len;
            if (s1->verbose == 2)
                printf("   -> %s\n", hdr.ar_name);
            if (swirl_load_object_file(s1, fd, off) < 0)
                goto the_end;
            ++bound;
        }
    } while(bound);
    ...
    for(;;) {
        ...
        if (alacarte) {
            ...
            if (!strcmp(hdr.ar_name, "/"))
                return swirl_load_alacarte(s1, fd, size, 4);
            if (!strcmp(hdr.ar_name, "/SYM64/"))
                return swirl_load_alacarte(s1, fd, size, 8);
        } else if (swirl_object_type(fd, &ehdr) == AFF_BINTYPE_REL) {
            if (s1->verbose == 2)
                printf("   -> %s\n", hdr.ar_name);
            if (swirl_load_object_file(s1, fd, file_offset) < 0)
                return -1;
        }
        file_offset += size;
    }
...
static void set_ver_to_ver(SwirlState *s1, int *n, int **lv, int i, char *lib, char *version)
{
    while (i >= *n) {
        *lv = swirl_realloc(*lv, (*n + 1) * sizeof(**lv));
        (*lv)[(*n)++] = -1;
    }
    if ((*lv)[i] == -1) {
        ...
        if (v == nb_sym_versions) {
            sym_versions = swirl_realloc (sym_versions,
                                        (v + 1) * sizeof(*sym_versions));
            sym_versions[v].lib = swirl_strdup(lib);
            sym_versions[v].version = swirl_strdup(version);
...
/* Associates symbol SYM_INDEX (in dynsymtab) with sym-version index
   VERNDX.  */
static void
set_sym_version(SwirlState *s1, int sym_index, int verndx)
{
    if (sym_index >= nb_sym_to_version) {
        int newelems = sym_index ? sym_index * 2 : 1;
        sym_to_version = swirl_realloc(sym_to_version,
...
static void store_version(SwirlState *s1, struct versym_info *v, char *dynstr)
{
    char *lib, *version;
    uint32_t next;
    int i;
...
ST_FUNC int swirl_load_dll(SwirlState *s1, int fd, const char *filename, int level)
{
    ...
    if (ehdr.e_ident[5] != ELFDATA2LSB ||
        ehdr.e_machine != EM_SWIRL_TARGET) {
        swirl_error_noabort("bad architecture");
        return -1;
    }
    ...
    soname = swirl_basename(filename);
    ...
    if (v.nb_versyms != nb_syms)
        swirl_free (v.versym), v.versym = NULL;
    else
        store_version(s1, &v, dynstr);

    /* add the dll and its level */
    swirl_add_dllref(s1, soname)->level = level;
    ...
    for(i = 0, dt = dynamic; i < nb_dts; i++, dt++) {
        ...
        case DT_NEEDED:
            ...
            if (swirl_add_dll(s1, name, AFF_REFERENCED_DLL) < 0) {
                swirl_error_noabort("referenced dll '%s' not found", name);
                ret = -1;
                goto the_end;
            }
        already_loaded:
 ...
 the_end:
    ...
    swirl_free(dynamic);
    swirl_free(shdr);
    swirl_free(v.local_ver);
    swirl_free(v.verdef);
    swirl_free(v.verneed);
    swirl_free(v.versym);
    return ret;
}
...
static int ld_inp(SwirlState *s1)
{
    char b;
    if (s1->cc != -1) {
        int c = s1->cc;
...
static int ld_next(SwirlState *s1, char *name, int name_size)
{
    int c, d, ch;
    char *q;
...
static int ld_add_file(SwirlState *s1, const char filename[])
{
    if (filename[0] == '/') {
        if (CONFIG_SYSROOT[0] == '\0'
            && swirl_add_file_internal(s1, filename, AFF_TYPE_BIN) == 0)
            return 0;
        filename = swirl_basename(filename);
    }
    return swirl_add_dll(s1, filename, 0);
}

static int ld_add_file_list(SwirlState *s1, const char *cmd, int as_needed)
{
    ...
    for(;;) {
        libname[0] = '\0';
        if (t == LD_TOK_EOF) {
            swirl_error_noabort("unexpected end of file");
            ret = -1;
            goto lib_parse_error;
        } else if (t == ')') {
            break;
            ...
            if ((t != LD_TOK_NAME) || (filename[0] != 'l')) {
                swirl_error_noabort("library name expected");
                ret = -1;
                goto lib_parse_error;
            }
            ...
            swirl_error_noabort("filename expected");
            ret = -1;
            goto lib_parse_error;
        }
        if (!strcmp(filename, "AS_NEEDED")) {
            ...
            if (!as_needed) {
                ...
                if (group) {
                    ...
                    dynarray_add(&libs, &nblibs, swirl_strdup(filename));
                    if (libname[0] != '\0')
                        dynarray_add(&libs, &nblibs, swirl_strdup(libname));
                }
            }
        }
        t = ld_next(s1, filename, sizeof(filename));
