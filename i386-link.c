...
ST_FUNC void relocate_plt(SwirlState *s1)
{
    ...
    if (s1->output_type != SWIRL_OUTPUT_DLL && p < p_end) {
...
void relocate(SwirlState *s1, ElfW_Rel *rel, int type, unsigned char *ptr, addr_t addr, addr_t val)
{
    ...
    switch (type) {
        ...
        case R_386_GOT32X:
            ...
            return;
        case R_386_16:
            if (s1->output_format != SWIRL_OUTPUT_FORMAT_BINARY) {
            output_file:
                swirl_error("can only produce 16-bit binary files");
            }
        ...
        case R_386_TLS_GD:
            {
                ...
                if (memcmp (ptr-3, expect, sizeof(expect)) == 0) {
                    ...
                    swirl_error("unexpected R_386_TLS_GD pattern");
            }
            return;
        case R_386_TLS_LDM:
            {
                ...
                if (memcmp (ptr-2, expect, sizeof(expect)) == 0) {
                    ...
                    swirl_error("unexpected R_386_TLS_LDM pattern");
            }
            return;
        case R_386_TLS_LDO_32:
        case R_386_TLS_LE:
