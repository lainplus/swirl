/*
 * Mach-O file handling for Swirl
 */
#include "swirl.h"
...
{
    struct load_command *lc = swirl_mallocz(cmdsize);
    lc->cmd = cmd;
    lc->cmdsize = cmdsize;
    mo->lc = swirl_realloc(mo->lc, sizeof(mo->lc[0]) * (mo->nlc + 1));
    mo->lc[mo->nlc++] = lc;
    return lc;
}
...
static int add_section(struct macho *mo, struct segment_command_64 **_seg, char *name)
{
    ...
    seg = swirl_realloc(seg, sizeof(*seg) + seg->nsects * sizeof(*sec));
...
static void check_relocs(SwirlState *s1, struct macho *mo)
{
    Section *s;
    ElfW_Rel *rel;
    ElfW(Sym) *sym;
            ...
            if (sym->st_shndx == SHN_UNDEF
                ...
                if (!attr->dyn_index) {
                    ...
                    if (ELFW(ST_BIND)(sym->st_info) == STB_LOCAL) {
                        if (sym->st_shndx == SHN_UNDEF)
                          swirl_error("undefined local symbol???");
...
static int check_symbols(SwirlState *s1, struct macho *mo)
{
    ...
    for (sym_index = 1; sym_index < sym_end; ++sym_index) {
        ...
        if (bind == STB_LOCAL) {
            ...
            if (mo->iextdef != -1 || mo->iundef != -1)
              swirl_error("local syms after global ones");
        } else if (sym->st_shndx != SHN_UNDEF) {
            if (mo->iextdef == -1)
              mo->iextdef = sym_index - 1;
            if (mo->iundef != -1)
              swirl_error("external defined symbol after undefined");
        } else if (sym->st_shndx == SHN_UNDEF) {
            if (mo->iundef == -1)
...
static void convert_symbol(SwirlState *s1, struct macho *mo, struct nlist_64 *pn)
{
    ...
    case STT_FILE:
        ...
        break;
    default:
        swirl_error("unhandled ELF symbol type %d %s",
                  ELFW(ST_TYPE)(sym->st_info), name);
    }
    if (sym->st_shndx == SHN_UNDEF)
      swirl_error("should have been rewritten to SHN_FROMDLL: %s", name);
    else if (sym->st_shndx == SHN_FROMDLL)
      n.n_type = N_UNDF, n.n_sect = 0;
    else if (sym->st_shndx == SHN_ABS)
      n.n_type = N_ABS, n.n_sect = 0;
    else if (sym->st_shndx >= SHN_LORESERVE)
      swirl_error("unhandled ELF symbol section %d %s", sym->st_shndx, name);
    else if (!mo->elfsectomacho[sym->st_shndx])
      swirl_error("ELF section %d not mapped into Mach-O for symbol %s",
                sym->st_shndx, name);
    else
...
static void convert_symbols(SwirlState *s1, struct macho *mo)
{
    struct nlist_64 *pn;
...
static void create_symtab(SwirlState *s1, struct macho *mo)
{
    int sym_index, sym_end;
    struct nlist_64 *pn;
    ...
    swirl_enter_state(s1);  /* qsort needs global state */
    qsort(pn, sym_end - 1, sizeof(*pn), machosymcmp);
    swirl_exit_state();
    mo->e2msym = swirl_malloc(sym_end * sizeof(*mo->e2msym));
    mo->e2msym[0] = -1;
    for (sym_index = 1; sym_index < sym_end; ++sym_index) {
...
static void collect_sections(SwirlState *s1, struct macho *mo)
{
    int i, sk, numsec;
    ...
    seg = NULL;
    numsec = 0;
    mo->elfsectomacho = swirl_mallocz(sizeof(*mo->elfsectomacho) * s1->nb_sections);
    for (sk = sk_unknown; sk < sk_last; sk++) {
        ...
        if (skinfo[sk].seg && mo->sk_to_sect[sk].s) {
            ...
            al = 1ULL << al;
            if (al > 4096)
              swirl_warning("alignment > 4096"), sec->align = 12, al = 4096;
...
static void macho_write(SwirlState *s1, struct macho *mo, FILE *fp)
{
    int i, sk;
    uint64_t fileofs = 0;
    Section *s;
...
ST_FUNC int macho_output_file(SwirlState *s1, const char *filename)
{
    ...
    if (fd < 0) {
        swirl_error_noabort("could not write '%s: %s'", filename, strerror(errno));
        return -1;
    }
    fp = fdopen(fd, "wb");
    if (s1->verbose)
 ...
 do_ret:
    for (i = 0; i < mo.nlc; i++)
      swirl_free(mo.lc[i]);
    swirl_free(mo.lc);
    swirl_free(mo.elfsectomacho);
    swirl_free(mo.e2msym);
...
ST_FUNC int macho_load_dll(SwirlState *s1, int fd, const char *filename, int lev)
{
  ...
  again:
    ...
    if (fh.magic == FAT_MAGIC || fh.magic == FAT_CIGAM) {
        ...
        swirl_warning("%s: Mach-O fat 64bit files of type 0x%x not handled",
                    filename, fh.magic);
        return -1;
    }
    ...
    for (i = 0, lc = buf2; i < mh.ncmds; i++) {
        ...
        case LC_REEXPORT_DYLIB:
        {
            ...
            dprintf(" REEXPORT %s\n", name);
            if (subfd < 0)
              swirl_warning("can't open %s (reexported from %s)", name, filename);
            else {
    ...
    dllref = swirl_mallocz(sizeof(DLLReference) + strlen(soname));
    dllref->level = lev;
    strcpy(dllref->name, soname);
    dynarray_add(&s1->loaded_dlls, &s1->nb_loaded_dlls, dllref);

    if (!nsyms || !nextdef)
      swirl_warning("%s doesn't export any symbols?", filename);
