...
do
	...
	do
		...
		swirl_object="${state}/swirl-$s $args.o"
		...
		then
			...
			if echo '__asm__("'"$s ${args}"'");'| ${SWIRL} -o "${swirl_object}" -c -
			then
				"${CROSS_COMPILE}objdump" -S "${swirl_object}" |grep "^[ ]*0:" >"${got}"
				if diff -u "${got}" "${expected}"
				then
					touch "${state}/ok-$s $args"
				else
					echo "warning: '$s $args' did not work in swirl (see above)">&2
				fi
			else
				rm -f "${swirl_object}"
				echo "warning: '$s $args' did not work in swirl">&2
			fi
			ok=1
		else # GNU as can't do it either--so i don't care
			rm -f "${as_object}"
		fi
		rm -f "${err}"
	done
