...
static void set_options(SwirlState *s, int argc, char **argv)
{
    int i;
    for (i = 1; i < argc; ++i) {
        char *a = argv[i];
...
static int run_callback(const char *src, callback_type callback) {
  ...
  if (swirl_set_output_type(s, SWIRL_OUTPUT_MEMORY) == -1)
    return -1;
  if (swirl_compile_string(s, src) == -1)
    return -1;
  if (swirl_relocate(s, SWIRL_RELOCATE_AUTO) == -1)
    return -1;
  
  ptr = swirl_get_symbol(s, "f");
  if (!ptr)
    return -1;
  result = callback(ptr);
 ...
 * address, thus causing a success message even though Swirl actually
