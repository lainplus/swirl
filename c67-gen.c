/*
 *  TMS320C67xx code generator for Swirl
 */
...
// although swirl thinks it is passing parameters on the stack,
...
do {\
   if (!(x))\
       swirl_error("internal compiler error file at %s:%d", __FILE__, __LINE__);\
} while (0)
...
// these are regs that swirl doesn't really know about, 
...
// map Swirl reg to C67 reg number

int C67_map_regn(int r)
{
    if (r == 0)			// normal swirl regs
	return 0x2;		// A2
    else if (r == 1)		// normal swirl regs
	return 3;		// A3
    else if (r == 2)		// normal swirl regs
	return 0;		// B0
    else if (r == 3)		// normal swirl regs
	return 1;		// B1
...
// mapping from swirl reg number to 
...
int C67_map_regc(int r)
{
	...
	return 0x2;
    else if (r == C67_B2)	// normal swirl regs
	return 0x3;
    else if (r == C67_CREG_ZERO)
	return 0;		// Special code for no condition reg test
    else
...
// map Swirl reg to C67 reg side A or B
...
void load(int r, SValue * sv)
{
	...
	if (v == VT_LLOCAL) {
	    ...
	    swirl_error("long double not supported");
	...
	// if it is swirl is referencing what it thinks is a parameter
...
void store(int r, SValue * v)
{
	...
	if ((v->r & VT_VALMASK) == VT_CONST) {
	    ...
	    // swirl thinks is on the stack but for C67 is
...
void gfunc_call(int nb_args)
{
	...
	swirl_error("more than 10 function params not currently supported");
	...
	if ((vtop->type.t & VT_BTYPE) == VT_STRUCT) {
		...
		swirl_error("long long not supported");
...
void gfunc_prolog(Sym *func_sym)
{
	...
	// we can translate where swirl thinks they
...
void gen_opf(int op)
{
	...
	swirl_error("long doubles not supported");
...
ST_FUNC void gen_vla_sp_save(int addr) {
    swirl_error("variable length arrays unsupported for this target");
}
