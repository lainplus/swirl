/*
 *  ARM specific functions for Swirl assembler
 */

#ifdef TARGET_DEFS_ONLY

#define CONFIG_SWIRL_ASM
...
static void parse_operand(SwirlState *s1, Operand *op)
{
    ExprValue e;
    int8_t reg;
    uint16_t regset = 0;
    ...
    if (tok == '{') { // regset literal
        ...
        while (tok != '}' && tok != TOK_EOF) {
            ...
            if ((1 << reg) < regset)
                swirl_warning("registers will be processed in ascending order by hardware--but are not specified in ascending order here");
            regset |= 1 << reg;
            if (tok != ',')
                break;
            next(); // skip ','
        ...
        if (regset == 0) {
            ...
            swirl_error("empty register list is not supported");
        } else {
            op->type = OP_REGSET32;
            op->regset = regset;
        }
...
static void asm_unary_opcode(SwirlState *s1, int token)
{
    Operand op;
    parse_operand(s1, &op);
    ...
    case TOK_ASM_swieq:
        if (op.type != OP_IM8)
            ...
            /* dummy operand (ignored by processor): ARM ref documented 0...255, ARM instruction set documented 24 bit */
...
static void asm_binary_opcode(SwirlState *s1, int token)
{
    Operand ops[2];
    Operand rotation;
    ...
    if (ops[0].reg == 15) {
        swirl_error("'%s' does not support 'pc' as operand", get_tok_str(token, NULL));
        return;
    }

    if (ops[0].reg == 13)
        swirl_warning("Using 'sp' as operand with '%s' is deprecated by ARM", get_tok_str(token, NULL));
    ...
    case TOK_ASM_clzeq:
        if (encoded_rotation)
            swirl_error("clz does not support rotation");
...
/* for data processing instructions, "1" means immediate.
   for single data transfer instructions, "0" means immediate. */
...
static void asm_block_data_transfer_opcode(SwirlState *s1, int token)
{
    uint32_t opcode;
    int op0_exclam;
    Operand ops[2];
    ...
    switch (ARM_INSTRUCTION_GROUP(token)) {
    case TOK_ASM_pusheq: // optimize 1-register case to: str ?, [sp, #-4]!
        ...
        break;
    case TOK_ASM_popeq: // optimize 1-register case to: ldr ?, [sp], #4
        ...
        if (nb_ops != 1)
            ...
            asm_emit_opcode(token, (0x8bd << 16) | ops[0].regset); // base register?
        break;
    ...
    case TOK_ASM_ldmibeq:
        ...
        case TOK_ASM_ldmibeq: // pre-increment load
            ...
            break;
        default:
            swirl_error("internal error: This place should not be reached (fallback in asm_block_data_transfer_opcode)");
        }
        // operands:
        //    Rn: first operand
        ...
        if (nb_ops != 2)
            ...
            swirl_error("first operand of '%s' should have an exclamation mark", get_tok_str(token, NULL));
        else
...
static uint32_t asm_parse_optional_shift(SwirlState* s1, int* nb_shift, Operand* shift)
{
    uint32_t opcode = 0;
    *nb_shift = 0;
    switch (tok) {
...
static uint32_t asm_encode_shift(Operand* shift)
{
    ...
    case OP_REG32:
        if (shift->reg == 15)
            swirl_error("r15 cannot be used as a shift count");
        else {
    ...
    case OP_IM8:
        ...
        if (amount > 0 && amount < 32)
            ...
            swirl_error("shift count out of range");
        break;
    default:
        swirl_error("unknown shift amount");
    }
    return operands;
}

static void asm_data_processing_opcode(SwirlState *s1, int token)
{
    Operand ops[3];
    int nb_ops;
    Operand shift = {};
    ...
    if (nb_ops < 2)
        ...
        if (opcode_nos == 0xd || opcode_nos == 0xf || opcode_nos == 0xa || opcode_nos == 0xb || opcode_nos == 0x8 || opcode_nos == 0x9) { // mov, mvn, cmp, cmn, tst, teq
            swirl_error("'%s' cannot be used with three operands", get_tok_str(token, NULL));
            return;
        }
    }
    if (nb_ops != 3) {
        ...
        if (nb_shift && shift.type == OP_REG32) {
            if ((ops[0].type == OP_REG32 && ops[0].reg == 15) ||
                ...
                swirl_error("Using the 'pc' register in data processing instructions that have a register-controlled shift is not implemented by ARM");
                return;
            }
        }
        ...
        case OP_IM8N: // immediate negative value
            ...
            */
            ...
            case 0xf: // MVN - Rd:= NOT Op2
                ...
                break;
            default:
                swirl_error("cannot use '%s' with a negative immediate value", get_tok_str(token, NULL));
            }
            for (half_immediate_rotation = 0; half_immediate_rotation < 16; ++half_immediate_rotation) {
                if (immediate_value >= 0x00 && immediate_value < 0x100)
                    break;
            ...
            if (half_immediate_rotation >= 16) {
                immediate_value = ops[2].e.v;
                swirl_error("immediate value 0x%X cannot be encoded into ARM immediate", (unsigned) immediate_value);
                return;
            }
            operands |= immediate_value;
            operands |= half_immediate_rotation << 8;
        ...
        if (nb_shift) {
            if (operands & ENCODE_IMMEDIATE_FLAG)
                swirl_error("immediate rotation not implemented");
            else
...
static void asm_shift_opcode(SwirlState *s1, int token)
{
    Operand ops[3];
    int nb_ops;
    ...
    if (nb_ops == 2) {
        ...
        case TOK_ASM_rrxeq:
            if (ops[1].type == OP_REG32) {
                ...
                swirl_error("(first source operand) register");
            return;
        default:
            memcpy(&ops[2], &ops[1], sizeof(ops[1])); // move ops[2]
            memcpy(&ops[1], &ops[0], sizeof(ops[0])); // ops[1] was implicit
...
static void asm_multiplication_opcode(SwirlState *s1, int token)
{
    Operand ops[4];
    int nb_ops = 0;
    ...
    if (nb_ops < 2)
        ...
        case TOK_ASM_mulseq:
        case TOK_ASM_muleq:
            memcpy(&ops[2], &ops[0], sizeof(ops[1])); // ARM is actually like this! Why!
            break;
        default:
...
static void asm_long_multiplication_opcode(SwirlState *s1, int token)
{
    Operand ops[4];
    int nb_ops = 0;
...
static void asm_single_data_transfer_opcode(SwirlState *s1, int token)
{
    Operand ops[3];
    ...
    if (tok == ',') {
        ...
        if (ops[2].type == OP_REG32) {
            if (ops[2].reg == 15) {
                swirl_error("Using 'pc' for register offset in '%s' is not implemented by ARM", get_tok_str(token, NULL));
                return;
            }
            if (tok == ',') {
                next();
    ...
    if (ops[2].type == OP_IM32 || ops[2].type == OP_IM8 || ops[2].type == OP_IM8N) {
        int v = ops[2].e.v;
        if (op2_minus)
            swirl_error("minus before '#' not supported for immediate values");
        if (v >= 0) {
            opcode |= 1 << 23; // up
            if (v >= 0x1000)
                swirl_error("offset out of range for '%s'", get_tok_str(token, NULL));
            else
                opcode |= v;
        } else { // down
            if (v <= -0x1000)
    ...
    case TOK_ASM_strexeq:
        if ((opcode & 0xFFF) || nb_shift) {
            swirl_error("neither offset nor shift allowed with 'strex'");
            return;
        } else if (opcode & ENCODE_IMMEDIATE_FLAG) { // if set, it means it's NOT immediate
            swirl_error("offset not allowed with 'strex'");
            return;
        }
        if ((opcode & (1 << 24)) == 0) { // add offset after transfer
            swirl_error("adding offset after transfer not allowed with 'strex'");
            return;
        }
    ...
    case TOK_ASM_ldrexeq:
        if ((opcode & 0xFFF) || nb_shift) {
            swirl_error("neither offset nor shift allowed with 'ldrex'");
            return;
        } else if (opcode & ENCODE_IMMEDIATE_FLAG) { // if set, it means it's NOT immediate
            swirl_error("offset not allowed with 'ldrex'");
            return;
        }
        if ((opcode & (1 << 24)) == 0) { // add offset after transfer
            swirl_error("adding offset after transfer not allowed with 'ldrex'");
            return;
        }
        opcode |= 1 << 20; // L
        opcode |= 0x00f;
...
static void asm_misc_single_data_transfer_opcode(SwirlState *s1, int token)
{
    Operand ops[3];
    int exclam = 0;
    int closed_bracket = 0;
    ...
    if (exclam) {
        if ((opcode & (1 << 24)) == 0) {
            swirl_error("result of '%s' would be unpredictable here", get_tok_str(token, NULL));
            return;
        }
        opcode |= 1 << 21; // write offset back into register
    }
...
static uint32_t encbranchoffset(int pos, int addr, int fail)
{
  ...
  if(addr>=0x7fffff || addr<-0x800000) {
    if(fail)
      swirl_error("branch offset is too far");
    return 0;
  }
  return /*not 0x0A000000|*/(addr&0xffffff);
}

static void asm_branch_opcode(SwirlState *s1, int token)
{
    int jmp_disp = 0;
    Operand op;
    ExprValue e;
    ...
    case TOK_ASM_bleq:
        ...
        if (!esym || esym->st_shndx != cur_text_section->sh_num) {
            swirl_error("invalid branch target");
            return;
        }
...
ST_FUNC void asm_opcode(SwirlState *s1, int token)
{
...
ST_FUNC void subst_asm_operand(CString *add_str, SValue *sv, int modifier)
{
    ...
    if ((r & VT_VALMASK) == VT_CONST) {
        ...
        if (r & VT_SYM) {
            ...
            if (swirl_state->leading_underscore)
                cstr_ccat(add_str, '_');
        ...
        for(i = 0; i < nb_operands; i++) {
            ...
            if (op->reg >= 0) {
                if ((op->vt->r & VT_VALMASK) == VT_LLOCAL &&
                    ...
                    load(op->reg, op->vt);
                    if (op->is_llong)
                        swirl_error("long long not implemented");
                }
            }
        }
    } else { // epilog
...
static inline int constraint_priority(const char *str)
{
    ...
    for(;;) {
        ...
        case 'g': // general-purpose-register, memory, immediate integer [general]
            ...
            break;
        default:
            swirl_error("unknown constraint '%c'", c);
            pr = 0;
        }
        if (pr > priority)
            priority = pr;
    ...
    for (i = 0; i < nb_operands; i++) {
        ...
        if (isnum(*str) || *str == '[') {
            ...
            if ((unsigned) k >= i || i < nb_outputs)
                swirl_error("invalid reference in constraint %d ('%s')",
                          i, str);
            op->ref_index = k;
            if (operands[k].input_index >= 0)
                swirl_error("cannot reference twice the same operand");
    ...
    for (i = 0; i < nb_operands; i++) {
      ...
      try_next:
        ...
        case '&': // Operand is clobbered before the instruction is done using the input operands
            if (j >= nb_outputs)
                swirl_error("'%c' modifier can only be applied to outputs",
                          c);
            reg_mask = REG_IN_MASK | REG_OUT_MASK;
            goto try_next;
        ...
        case 'g':
            ...
            break;
        default:
            swirl_error("asm constraint %d ('%s') could not be satisfied",
                      j, op->constraint);
            break;
        }
    ...
    *pout_reg = -1;
    for (i = 0; i < nb_operands; i++) {
        ...
        if (op->reg >= 0 &&
            ...
            swirl_error("could not find free output register for reloading");
          reg_found2:
            *pout_reg = reg;
            break;
        }
...
ST_FUNC void asm_clobber(uint8_t *clobber_regs, const char *str)
{
    ...
    if (reg == -1) {
        swirl_error("invalid clobber register '%s'", str);
    }
    clobber_regs[reg] = 1;
}
