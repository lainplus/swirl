/*
 *  ARMv4 code generator for Swirl
 */

#ifdef TARGET_DEFS_ONLY

#if defined(SWIRL_ARM_EABI) && !defined(SWIRL_ARM_VFP)
#error "Currently Swirl only supports float computation with VFP instructions"
#endif
...
#ifndef SWIRL_CPU_VERSION
# define SWIRL_CPU_VERSION 5
#endif
...
#if defined(CONFIG_SWIRL_BCHECK)
...
{
#if 0
#if !defined (SWIRL_ARM_VFP)
    swirl_warning("Support for FPA is deprecated and will be removed in next"
                " release");
#endif
#if !defined (SWIRL_ARM_EABI)
    swirl_warning("Support for OABI is deprecated and will be removed in next"
                " release");
#endif
#endif
}
...
static int two2mask(int a,int b) {
  if (!CHECK_R(a) || !CHECK_R(b))
    swirl_error("compiler error! registers %i,%i is not valid",a,b);
  return (reg_classes[a]|reg_classes[b])&~(RC_INT|RC_FLOAT);
}

static int regmask(int r) {
  if (!CHECK_R(r))
    swirl_error("compiler error! register %i is not valid",r);
...
const char *default_elfinterp(struct SwirlState *s)
{
...
void o(uint32_t i)
{
  ...
  ind1 = ind + 4;
  if (!cur_text_section)
    swirl_error("compiler error! This happens f.ex. if the compiler\n"
...
uint32_t encbranch(int pos, int addr, int fail)
{
  ...
  if(addr>=0x1000000 || addr<-0x1000000) {
    if(fail)
      swirl_error("FIXME: function bigger than 32MB");
    return 0;
  }
...
static uint32_t vfpr(int r)
{
  if(r<TREG_F0 || r>TREG_F7)
    swirl_error("compiler error! register %i is no vfp register",r);
  return r - TREG_F0;
}
#else
static uint32_t fpr(int r)
{
  if(r<TREG_F0 || r>TREG_F3)
    swirl_error("compiler error! register %i is no fpa register",r);
  return r - TREG_F0;
}
#endif
...
static uint32_t intr(int r)
{
  ...
  if (!(r >= TREG_SP && r <= TREG_LR))
    swirl_error("compiler error! register %i is no int register",r);
  return r + (13 - TREG_SP);
}
...
static uint32_t mapcc(int cc)
{
  ...
  swirl_error("unexpected condition code");
...
void load(int r, SValue *sv)
{
  ...
  swirl_error("load unimplemented!");
}
...
void store(int r, SValue *sv)
{
  ...
  swirl_error("store unimplemented");
}
...
/* Assign parameters to registers and stack with alignment according to the
   ...
   Note: this function allocated an array in plan->pplans with swirl_malloc. It
...
again:
      ...
      switch(i) {
        ...
        case VFP_STRUCT_CLASS:
          if ((pplan->sval->type.t & VT_BTYPE) == VT_STRUCT) {
            ...
            vswap();
	    /* optimize. Save all register because memcpy can use them */
  ...
  /* second pass to restore registers that were saved on stack by accident.
     Maybe redundant after the "lvalue_save" patch in swirlgen.c:gv() */
  if (++pass < 2)
    goto again;
...
void gfunc_call(int nb_args)
{
  ...
  if (nb_args)
    plan.pplans = swirl_malloc(nb_args * sizeof(*plan.pplans));
...
{
    if ((bytes & 3))
      swirl_error("alignment of code section not multiple of 4");
    while (bytes > 0) {
	o(0xE1A00000);
	bytes -= 4;
    }
...
void gen_opi(int op)
{
  ...
  switch(c) {
    ...
    case 3:
      ...
      break;
    default:
      swirl_error("gen_opi %i unimplemented!",op);
  }
}
...
void gen_opf(int op)
{
  ...
  switch(op) {
    ...
    default:
      if(op < TOK_ULT || op > TOK_GT) {
        swirl_error("unknown fp op %x!",op);
        return;
      }
      if(is_zero(-1)) {
        vswap();
...
/* bug (intention?) in Linux FPU emulator
   doesn't set carry if equal */
	switch(op) {
	  ...
	  case TOK_ULE:
	  case TOK_UGT:
            swirl_error("unsigned comparison on floats?");
	    break;
	  case TOK_LT:
            op=TOK_Nset;
	    break;
...
ST_FUNC void gen_cvt_itof(int t)
{
  ...
  swirl_error("unimplemented gen_cvt_itof %x!",vtop->type.t);
}
...
void gen_cvt_ftoi(int t)
{
  ...
  swirl_error("unimplemented gen_cvt_ftoi!");
}
...
#endif
    if (align & (align - 1))
        swirl_error("alignment is not a power of 2: %i", align);
