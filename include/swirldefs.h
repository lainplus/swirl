/*  swirldefs.h
    ...
    and the few things related to option settings in swirlpp.c:swirl_predefs().

    This file is either included at runtime as is, or converted and
    included as C-strings at compile-time (depending on CONFIG_SWIRL_PREDEFS).

    Note that line indent matters:

    - in lines starting at column 1, platform macros are replaced by
      corresponding swirl target compile-time macros.  See conftest.c for
...
# endif
    ...
    /* This should be in sync with the declaration in our lib/libswirl1.c */
    typedef struct {
    ...
    typedef char *__builtin_va_list;
    #define _swirl_alignof(type) ((int)&((struct {char c;type x;} *)0)->x)
    #define _swirl_align(addr,type) (((unsigned)addr + _swirl_alignof(type) - 1) \
                                  & ~(_swirl_alignof(type) - 1))
    #define __builtin_va_start(ap,last) (ap = ((char *)&(last)) + ((sizeof(last)+3)&~3))
    #define __builtin_va_arg(ap,type) (ap = (void *) ((_swirl_align(ap,type)+sizeof(type)+3) \
    ...
    #define _swirl_align(addr,type) (((unsigned long)addr + __alignof__(type) - 1) \
                                  & -(__alignof__(type)))
    #define __builtin_va_arg(ap,type) (*(sizeof(type) > (2*__va_reg_size) ? *(type **)((ap += __va_reg_size) - __va_reg_size) : (ap = (va_list)(_swirl_align(ap,type) + (sizeof(type)+__va_reg_size - 1)& -__va_reg_size), (type *)(ap - ((sizeof(type)+ __va_reg_size - 1)& -__va_reg_size)))))
    ...
    /* SWIRL BBUILTIN AND BOUNDS ALIASES */
